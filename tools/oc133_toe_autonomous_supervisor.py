    ...
    for row in rows:
        ...
        if source_node_id == "required_artifact:MODERN_SCIENCE_COMPARATOR_SUPERIORITY:source_implementation_backlog":
            ...
            if len(parts) >= 4:
                ...
                if artifact_key.startswith("source_implementation::"):
                ...
                elif artifact_key == "domain_scorer_implementation_backlog" and (root / factory.comparator_domain_scorer_implementation_backlog_rel()).exists():
                    payload["status"] = "PASS"
                    payload["superseded_by_domain_scorer_implementation_backlog"] = True
                    payload["capability_executor_ready"] = False
                    payload["execution_command"] = []
                    payload["implementation_command"] = []
                    payload["next_escalation"] = "Domain-scorer implementation backlog exists; execute the exact scorer implementation row next."
                elif artifact_key == "domain_scorer_implementation":
                    implementation_id = factory.comparator_domain_scorer_implementation_id(gap_id)
                    ...
                    if implementation_report.get("root_cause_class") == "DOMAIN_SCORER_MODEL_OR_COMPARATOR_REPAIR_REQUIRED":
                        ...
                        repair_report = factory.comparator_domain_model_repair_report(root, repair_id)
                        if repair_report:
                            payload["status"] = "PASS"
                            payload["superseded_by_domain_model_repair_report"] = True
                            payload["capability_executor_ready"] = False
                            payload["execution_command"] = []
                            payload["implementation_command"] = []
                            payload["next_escalation"] = "Domain model repair report exists; downstream scoring/replay artifacts decide whether broad superiority remains blocked."
                        elif (root / factory.comparator_domain_model_repair_backlog_rel()).exists():
                            payload["status"] = "PASS"
                            payload["superseded_by_domain_model_repair_backlog"] = True
                            payload["capability_executor_ready"] = False
                            payload["execution_command"] = []
                            payload["implementation_command"] = []
                            payload["next_escalation"] = "Domain model repair backlog exists; execute the exact model/comparator repair row next."
                    elif factory.comparator_domain_scorer_implementation_completed(root, implementation_id):
                        payload["status"] = "PASS"
                        payload["superseded_by_domain_scorer_implementation_report"] = True
                        payload["capability_executor_ready"] = False
                        payload["execution_command"] = []
                        payload["implementation_command"] = []
                    ...
                    if factory.comparator_component_materialization_report(root, materialization_id):
                        ...
                        payload["execution_command"] = []
                        payload["implementation_command"] = []
                        payload["next_escalation"] = "Component source-obligation report exists; downstream scoring/replay artifacts decide whether broad superiority remains blocked."
                    elif obligation_id in factory.comparator_component_source_obligation_rows_by_id(root):
                        payload["status"] = "PASS"
                        payload["superseded_by_component_source_obligation_backlog"] = True
                        payload["capability_executor_ready"] = False
                        payload["execution_command"] = []
                        payload["implementation_command"] = []
                        payload["next_escalation"] = "Component source-obligation backlog exists; execute the exact source-bound materialization row next."
                    elif factory.comparator_domain_model_component_implementation_report(root, implementation_id):
                        payload["status"] = "PASS"
...
def comparator_actions(root: Path) -> list[dict[str, Any]]:
    ...
    domain_scoring_rows = [
        row
        for row in factory.comparator_domain_scoring_rows(root)
    ...
    if domain_scoring_backlog.get("status") != "PASS" and domain_scoring_rows and not domain_scoring_reports_exist:
        ...
        if action_id not in seen_action_ids:
            ...
            action.update(
                {
                    "required_artifact": "domain_scoring_backlog",
                    "missing_artifact_type": "domain_scoring_backlog",
                    "pending_domain_scoring_total": len(domain_scoring_rows),
                    "why_it_failed": "Fail-closed strict evidence packs need domain-specific scoring executors.",
                    "repair_strategy": "Compile every fail-closed scoring artifact into an exact domain-scoring work order before broad superiority can advance.",
                    "required_capability": "Research/DomainScoringExecutor",
                    "pass_predicate": "Domain-scoring backlog exists and every row has an executable scorer or a narrower domain-scorer implementation obligation.",
                    "next_escalation": "Execute the first domain-scoring row; if it reports missing flags/script/model support, compile domain-scorer implementation backlog.",
                    "validator_binding": "comparator_domain_scoring_backlog",
                }
            )
            seen_action_ids.add(action_id)
            actions.append(action)
    for row in sorted(domain_scoring_rows, key=lambda item: (str(item.get("gap_id")), str(item.get("domain_scoring_id")))):
        scoring_id = str(row.get("domain_scoring_id") or "")
        gap_id = str(row.get("gap_id") or "")
        if not scoring_id or not gap_id or factory.comparator_domain_scoring_completed(root, scoring_id):
            continue
        scoring_report = read_json(root / factory.comparator_domain_scoring_rel(scoring_id))
        if scoring_report.get("status") in {"CAPABILITY_DEVELOPMENT_REQUIRED", "EVIDENCE_REPAIR_ATTEMPTED_REMAINS_OPEN", "FAIL_CLOSED"}:
            continue
        action_id = f"AUTO-R017-COMPARATOR-DOMAIN-SCORING-{artifact_hash({'scoring_id': scoring_id})[:12]}"
        if action_id in seen_action_ids:
            continue
        action = action_defaults(
            action_id,
        ...
        action.update(
            {
                ...
                "domain_class_id": row.get("domain_class_id"),
                "phenomenon_class_id": row.get("phenomenon_class_id"),
                "required_artifact": "domain_scoring_executor",
                "missing_artifact_type": "domain_scoring_executor",
                "source_ref": row.get("research_artifact_ref"),
                "why_it_failed": row.get("why_it_failed"),
                "repair_strategy": row.get("repair_strategy"),
                "required_capability": "Research/DomainScoringExecutor",
                "pass_predicate": row.get("pass_predicate"),
                "next_escalation": "If this scorer execution produces no validator delta, compile a domain-scorer implementation obligation from its exact root cause.",
                "validator_binding": row.get("validator_binding"),
            }
        )
        seen_action_ids.add(action_id)
        actions.append(action)
    domain_scorer_backlog = read_json(root / factory.comparator_domain_scorer_implementation_backlog_rel())
    pending_domain_scorer_rows = factory.comparator_domain_scorer_implementation_rows(root)
    scorer_trigger_reports = [
        read_json(root / factory.comparator_domain_scoring_rel(str(row.get("domain_scoring_id") or "")))
        for row in domain_scoring_rows
        if row.get("domain_scoring_id")
    ]
    needs_scorer_impl = any(
        report.get("status") in {"CAPABILITY_DEVELOPMENT_REQUIRED", "EVIDENCE_REPAIR_ATTEMPTED_REMAINS_OPEN", "FAIL_CLOSED"}
    ...
    if needs_scorer_impl and domain_scorer_backlog.get("status") != "PASS" and not pending_domain_scorer_rows:
        ...
        if action_id not in seen_action_ids:
            ...
            action.update(
                {
                    "required_artifact": "domain_scorer_implementation_backlog",
                    "missing_artifact_type": "domain_scorer_implementation_backlog",
                    "pending_domain_scorer_implementation_total": len(pending_domain_scorer_rows),
                    "why_it_failed": "Domain-scoring reports show missing scorer scripts/flags or scorer/model failures.",
                    "repair_strategy": "Compile exact domain-scorer implementation obligations from the current scoring report root causes.",
                    "required_capability": "Research/DomainScorerImplementation",
                    "pass_predicate": "Domain-scorer implementation backlog exists and names exact script/flag/model repair contracts.",
                    "next_escalation": "Execute the first domain-scorer implementation row; if source code is absent, implement that specific script/flag contract.",
                    "validator_binding": "comparator_domain_scorer_implementation_backlog",
                }
            )
            seen_action_ids.add(action_id)
            actions.append(action)
    for row in sorted(pending_domain_scorer_rows, key=lambda item: (str(item.get("gap_id")), str(item.get("implementation_id")))):
        implementation_id = str(row.get("implementation_id") or "")
        if not implementation_id or factory.comparator_domain_scorer_implementation_completed(root, implementation_id):
            continue
        gap_id = str(row.get("gap_id") or "")
        implementation_report = factory.comparator_domain_scorer_implementation_report(root, implementation_id)
        if implementation_report.get("root_cause_class") == "DOMAIN_SCORER_MODEL_OR_COMPARATOR_REPAIR_REQUIRED":
            ...
            action = action_defaults(
                ...
                if repair_id in factory.comparator_domain_model_repair_rows_by_id(root)
                else [sys.executable, "tools/oc133_toe_closure_factory.py", "--compile-comparator-domain-model-repair-backlog", "--write"],
            )
            action.update(
                {
                    "repair_id": repair_id,
                    "implementation_id": implementation_id,
                    "domain_scoring_id": row.get("domain_scoring_id"),
                    "gap_id": gap_id,
                    "domain_class_id": row.get("domain_class_id"),
                    "phenomenon_class_id": row.get("phenomenon_class_id"),
                    "required_artifact": "domain_model_repair",
                    "missing_artifact_type": "domain_model_repair",
                    "source_ref": implementation_report.get("artifact_ref") or row.get("source_domain_scoring_ref"),
                    "why_it_failed": "The domain scorer has run; the remaining blocker is OC/comparator residual/materiality model repair, not scorer-route implementation.",
                    "repair_strategy": "Compile and execute component-level model/comparator repair obligations from the scorer report.",
                    "required_capability": "Research/DomainModelComparatorRepair",
                    "pass_predicate": "The downstream oc_prediction_scoring_row and replay_record artifacts become PASS without weakening materiality.",
                    "next_escalation": "If this produces zero validator delta, split the model repair row into OC-score, comparator-score, residual, materiality, and replay component work.",
            ...
            actions.append(action)
            continue
        action_id = f"AUTO-R017-COMPARATOR-DOMAIN-SCORER-IMPLEMENTATION-{artifact_hash({'implementation_id': implementation_id})[:12]}"
        if action_id in seen_action_ids:
            continue
        action = action_defaults(
            action_id,
        ...
        action.update(
            {
                ...
                "domain_class_id": row.get("domain_class_id"),
                "phenomenon_class_id": row.get("phenomenon_class_id"),
                "required_artifact": "domain_scorer_implementation",
                "missing_artifact_type": "domain_scorer_implementation",
                "source_ref": row.get("source_domain_scoring_ref"),
                "why_it_failed": row.get("why_it_failed"),
                "repair_strategy": row.get("repair_strategy"),
                "required_capability": "Research/DomainScorerImplementation",
                "pass_predicate": row.get("pass_predicate"),
        ...
        seen_action_ids.add(action_id)
        actions.append(action)
    domain_model_backlog = read_json(root / factory.comparator_domain_model_repair_backlog_rel())
    domain_model_reports_open = [
        read_json(path)
        for path in (root / factory.lane_execution_base("MODERN_SCIENCE_COMPARATOR_SUPERIORITY") / "domain_model_repairs").glob("*.json")
    ...
    if (
        ...
        if action_id not in seen_action_ids:
            ...
            action.update(
                {
                    "required_artifact": "domain_model_component_backlog",
                    "missing_artifact_type": "domain_model_component_backlog",
                    "pending_domain_model_repair_total": len(domain_model_reports_open),
                    "why_it_failed": "Domain-model repair reports contain open component obligations.",
                    "repair_strategy": "Compile model/comparator repair components into exact implementation obligations.",
                    "required_capability": "Research/DomainModelComparatorRepair",
                    "pass_predicate": "Component backlog exists and every open component has a source-bound work order.",
                    "next_escalation": "Execute the first component row and implement its domain scorer/model code if still open.",
                    "validator_binding": "comparator_domain_model_component_backlog",
                }
            )
            seen_action_ids.add(action_id)
            actions.append(action)
    ...
    if (
        ...
        if action_id not in seen_action_ids:
            ...
            action.update(
                {
                    "required_artifact": "component_source_obligation_backlog",
                    "missing_artifact_type": "component_source_obligation_backlog",
                    "pending_component_implementation_open_total": len(component_implementation_reports_evidence_open),
                    "pending_component_source_obligation_total": len(pending_component_source_rows),
                    "why_it_failed": "Component implementation commands have run, but downstream source-bound scoring/replay evidence remains open.",
                    "repair_strategy": "Compile exhausted component implementations into exact source/model/comparator/residual/replay obligations before rerunning implementation commands.",
                    "required_capability": "Research/SourceBoundScoringMaterialization",
                    "pass_predicate": "Component source obligation backlog exists and every command-ran-but-evidence-open component is represented.",
                    "next_escalation": "Execute the first component source obligation and inspect the missing source evidence fields.",
                    "validator_binding": "comparator_component_source_obligation_backlog",
                }
            )
            seen_action_ids.add(action_id)
            actions.append(action)
        ...
        action.update(
            {
                ...
                "phenomenon_class_id": row.get("phenomenon_class_id"),
                "component_id": row.get("component_id"),
                "required_artifact": f"component_source_obligation::{row.get('component_id')}",
                "missing_artifact_type": f"component_source_obligation::{row.get('component_id')}",
                "source_ref": row.get("source_component_implementation_ref"),
                "missing_source_evidence_fields": row.get("missing_source_evidence_fields"),
                "why_it_failed": row.get("why_it_failed"),
                "repair_strategy": row.get("repair_strategy"),
                "required_capability": row.get("required_capability"),
        ...
        action.update(
            {
                ...
                "phenomenon_class_id": row.get("phenomenon_class_id"),
                "component_id": row.get("component_id"),
                "required_artifact": f"domain_model_component::{row.get('component_id')}",
                "missing_artifact_type": f"domain_model_component::{row.get('component_id')}",
                "source_ref": row.get("source_domain_model_repair_ref"),
                "why_it_failed": row.get("why_it_failed"),
                "repair_strategy": row.get("repair_strategy"),
                "required_capability": row.get("required_capability"),
                "pass_predicate": row.get("pass_predicate"),
            ...
            if (
                ...
                if factory.comparator_gap_research_artifact_passes(ROOT, gap_id, artifact_key):
                    ...
                    for stale_flag in [
                        ...
                        "superseded_by_domain_scorer_implementation_backlog",
                    ...
                    payload["execution_command"] = []
                    payload["implementation_command"] = []
                    payload["next_escalation"] = "Scoring work order now exists for this scope; advance to its lower-level source/evidence subartifact executors."
                    ...
                    payload["execution_command"] = []
                    payload["implementation_command"] = []
                    payload["next_escalation"] = "Domain-scorer implementation backlog exists; remaining work is the exact domain/phenomenon scorer source-code or model repair row."
                elif artifact_key == "domain_scorer_implementation":
                    implementation_id = factory.comparator_domain_scorer_implementation_id(gap_id)
                    implementation_report = factory.comparator_domain_scorer_implementation_report(ROOT, implementation_id)
                    if implementation_report.get("root_cause_class") == "DOMAIN_SCORER_MODEL_OR_COMPARATOR_REPAIR_REQUIRED":
                    ...
                    if factory.comparator_component_materialization_report(ROOT, materialization_id):
                        ...
                        payload["execution_command"] = []
                        payload["implementation_command"] = []
                        payload["next_escalation"] = "Component source-obligation backlog exists; remaining work is the exact source-bound materialization row."
                    elif factory.comparator_domain_model_component_implementation_report(ROOT, implementation_id):
                        payload["status"] = "PASS"
                        payload["superseded_by_domain_model_component_implementation_report"] = True
                        payload["capability_executor_ready"] = False
