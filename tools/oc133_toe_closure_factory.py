...
COMPARATOR_DOMAIN_SCRIPT_BY_CLASS = {
    "physical_sciences": "tools/oc133_physics_chemistry_official_batch_factory.py",
    "chemical_sciences": "tools/oc133_chemistry_pubchem_formula_batch_factory.py",
    ...
    "medical_health_sciences": "validation/heldout/grand_science/medical/coverage_work_orders/oc133_medical_modern_science_coverage_work_orders.py",
...
NIST_WEBBOOK_THERMO_MATERIALIZER = (
...
for _nist_thermo_subartifact in (
    "target_hidden_task_table",
    "oc_formula_or_model",
    "incumbent_comparator_scoring",
    "residuals_materiality_uncertainty",
...
def comparator_domain_script_base_commands(domain_class_id: str) -> list[list[str]]:
    ...
    if not script_ref:
        return []
    if domain_class_id == "physical_sciences":
        return [
            [sys.executable, script_ref, "--write", "--allow-blocked-exit-zero"],
            [sys.executable, script_ref, "--check"],
        ]
    if domain_class_id == "chemical_sciences":
        return [
            [sys.executable, script_ref, "--write", "--allow-blocked-exit-zero"],
            [sys.executable, script_ref, "--check"],
        ]
    if domain_class_id == "medical_health_sciences":
        return [
            [sys.executable, script_ref, "--write"],
            [sys.executable, script_ref, "--check"],
        ]
    return [
        [sys.executable, script_ref, "--write"],
...
def unique_commands(commands: list[list[str]]) -> list[list[str]]:
    ...
    for command in commands:
        ...
        if key in seen:
            continue
        seen.add(key)
        result.append([str(part) for part in command])
    return result


def comparator_domain_implemented_commands(
    domain_class_id: str,
    ...
    if exact:
        return unique_commands([list(command) for command in exact])
    base = comparator_domain_script_base_commands(domain_class_id)
    if not base:
        return []
    commands = [list(command) for command in base]
    if gap_id and subartifact_id in {
        "oc_formula_or_model",
        "incumbent_comparator_scoring",
        "residuals_materiality_uncertainty",
        "controls_and_falsifiers",
...
COMPONENT_TO_SCORING_SUBARTIFACTS = {
    "oc_score_materialization": [
        "target_hidden_task_table",
        "oc_formula_or_model",
        "strict_evidence_pack_diagnosis",
    ],
...
def comparator_domain_model_component_commands(
    domain_class_id: str,
    phenomenon_class_id: str,
    component_id: str,
    gap_id: str = "",
) -> list[list[str]]:
    subartifact_ids = COMPONENT_TO_SCORING_SUBARTIFACTS.get(component_id, [])
...
def comparator_source_executor_work_order_rel(gap_id: str, subartifact_id: str) -> Path:
    payload_hash = artifact_hash({"gap_id": gap_id, "subartifact_id": subartifact_id})[:16]
    return (
        lane_execution_base("MODERN_SCIENCE_COMPARATOR_SUPERIORITY")
        / "source_executor_work_orders"
        / f"source_executor_{payload_hash}.json"
    )
...
def comparator_domain_model_repair_backlog_rel() -> Path:
    return (
        lane_execution_base("MODERN_SCIENCE_COMPARATOR_SUPERIORITY")
        / "DOMAIN_MODEL_REPAIR_BACKLOG.json"
    )


def comparator_domain_model_repair_id(gap_id: str) -> str:
    return f"R017-DOMAIN-MODEL-REPAIR-{artifact_hash({'gap_id': gap_id})[:16]}"


def comparator_domain_model_repair_rel(repair_id: str) -> Path:
    safe_id = artifact_hash({"repair_id": repair_id})[:16]
    return (
        lane_execution_base("MODERN_SCIENCE_COMPARATOR_SUPERIORITY")
        / "domain_model_repairs"
        / f"model_repair_{safe_id}.json"
    )


def comparator_domain_model_component_backlog_rel() -> Path:
    return (
        lane_execution_base("MODERN_SCIENCE_COMPARATOR_SUPERIORITY")
        / "DOMAIN_MODEL_COMPONENT_BACKLOG.json"
    )


def comparator_domain_model_component_id(gap_id: str, component_id: str) -> str:
    return f"R017-DOMAIN-MODEL-COMPONENT-{artifact_hash({'gap_id': gap_id, 'component_id': component_id})[:16]}"


def comparator_domain_model_component_rel(component_work_order_id: str) -> Path:
    safe_id = artifact_hash({"component_work_order_id": component_work_order_id})[:16]
    return (
        lane_execution_base("MODERN_SCIENCE_COMPARATOR_SUPERIORITY")
        / "domain_model_components"
        / f"component_{safe_id}.json"
    )


def comparator_domain_model_component_implementation_backlog_rel() -> Path:
    return (
        lane_execution_base("MODERN_SCIENCE_COMPARATOR_SUPERIORITY")
        / "DOMAIN_MODEL_COMPONENT_IMPLEMENTATION_BACKLOG.json"
    )


def comparator_domain_model_component_implementation_registry_rel() -> Path:
    return (
        lane_execution_base("MODERN_SCIENCE_COMPARATOR_SUPERIORITY")
        / "DOMAIN_MODEL_COMPONENT_IMPLEMENTATION_REGISTRY.json"
    )


def comparator_domain_model_component_implementation_id(component_work_order_id: str) -> str:
    return f"R017-DOMAIN-MODEL-COMPONENT-IMPL-{artifact_hash({'component_work_order_id': component_work_order_id})[:16]}"


def comparator_domain_model_component_implementation_rel(implementation_id: str) -> Path:
    safe_id = artifact_hash({"implementation_id": implementation_id})[:16]
    return (
        lane_execution_base("MODERN_SCIENCE_COMPARATOR_SUPERIORITY")
        / "component_impls"
        / f"impl_{safe_id}.json"
    )


def comparator_component_source_obligation_backlog_rel() -> Path:
    return (
        lane_execution_base("MODERN_SCIENCE_COMPARATOR_SUPERIORITY")
        / "COMPONENT_SOURCE_OBLIGATION_BACKLOG.json"
    )


def comparator_component_source_obligation_registry_rel() -> Path:
    return (
        lane_execution_base("MODERN_SCIENCE_COMPARATOR_SUPERIORITY")
        / "COMPONENT_SOURCE_OBLIGATION_REGISTRY.json"
    )
...
def comparator_component_source_obligation_rel(obligation_id: str) -> Path:
    safe_id = artifact_hash({"obligation_id": obligation_id})[:16]
    return (
        lane_execution_base("MODERN_SCIENCE_COMPARATOR_SUPERIORITY")
        / "component_src"
...
def materialized_evidence_residuals(evidence_pack: dict[str, Any]) -> dict[str, Any]:
    model = first_numeric(
        ...
        [
            ...
            ["residuals", "model"],
            ["residuals", "model_relative_mae"],
            ["residuals", "model_abs_mae"],
            ["aggregate", "model_mae"],
            ["aggregate", "model_relative_mae"],
            ["aggregate", "model_abs_mae"],
    ...
    comparator = first_numeric(
        ...
        [
            ...
            ["residuals", "comparator_relative_mae"],
            ["residuals", "comparator_abs_mae"],
            ["aggregate", "comparator_mae"],
            ["aggregate", "comparator_relative_mae"],
            ["aggregate", "comparator_abs_mae"],
...
def comparator_scoring_subartifact_executions(root: Path) -> list[dict[str, Any]]:
    base = root / lane_execution_base("MODERN_SCIENCE_COMPARATOR_SUPERIORITY") / "scoring_subartifact_jobs"
    rows: list[dict[str, Any]] = []
    if base.exists():
        for path in sorted(base.glob("*.json")):
            payload = read_json(path)
...
def comparator_source_executor_work_order_exists(root: Path, gap_id: str, subartifact_id: str) -> bool:
    payload = read_json(root / comparator_source_executor_work_order_rel(gap_id, subartifact_id))
    return (
        payload.get("schema_id") == "OC133_MODERN_SCIENCE_COMPARATOR_SOURCE_EXECUTOR_WORK_ORDER_v1"
        and payload.get("gap_id") == gap_id
        and payload.get("scoring_subartifact_id") == subartifact_id
    )
...
def comparator_research_artifact_repair_commands(root: Path, gap_id: str, artifact_key: str) -> list[list[str]]:
    ...
    seen: set[str] = set()
    for subartifact_id in subartifact_ids:
        for command in comparator_domain_implemented_commands(domain_class_id, phenomenon_class_id, subartifact_id, gap_id):
...
def build_comparator_domain_scoring_execution(root: Path, scoring_id: str) -> dict[str, Any]:
    ...
    for subartifact_id in ("strict_evidence_pack_diagnosis", "model_or_claim_repair_decision", "oc_formula_or_model"):
        exact_commands.extend(comparator_domain_implemented_commands(domain_class_id, phenomenon_class_id, subartifact_id, gap_id))
    exact_commands = unique_commands(exact_commands)
...
def build_comparator_domain_scorer_implementation_backlog(root: Path) -> dict[str, Any]:
    ...
    for scoring_row in scoring_backlog.get("rows", []) or []:
        ...
        exact_commands = unique_commands([
            list(command)
...
def comparator_domain_model_repair_rows(root: Path) -> list[dict[str, Any]]:
    payload = read_json(root / comparator_domain_model_repair_backlog_rel())
    rows = payload.get("rows") or []
    return [row for row in rows if isinstance(row, dict)]


def comparator_domain_model_repair_rows_by_id(root: Path) -> dict[str, dict[str, Any]]:
    return {
        str(row.get("repair_id")): row
        for row in comparator_domain_model_repair_rows(root)
        if row.get("repair_id")
    }


def comparator_domain_model_repair_report(root: Path, repair_id: str) -> dict[str, Any]:
    payload = read_json(root / comparator_domain_model_repair_rel(repair_id))
    return payload if payload.get("repair_id") == repair_id else {}


def comparator_domain_model_repair_completed(root: Path, repair_id: str) -> bool:
    payload = comparator_domain_model_repair_report(root, repair_id)
    return payload.get("status") == "PASS" and payload.get("scoring_artifact_status") == "PASS"


def comparator_domain_model_repair_component_rows(
    gap_id: str,
    domain_class_id: str,
    phenomenon_class_id: str,
    ...
    residuals = summary.get("residuals", {}) if isinstance(summary.get("residuals"), dict) else {}
    checks = {
        "oc_score_materialization": summary.get("score_materialized") is True,
        "oc_model_residual": isinstance(residuals.get("model"), (int, float)),
        "incumbent_comparator_residual": isinstance(residuals.get("comparator"), (int, float)),
        "material_margin": residuals.get("material_margin_met") is True,
        "fail_closed_status_clear": validation.get("fail_closed_status_present") is False,
        "strict_pack_pass": evidence_pack.get("pack_status") == "PASS",
    }
    rows = []
    for component_id, passed in checks.items():
        if passed:
            continue
        rows.append(
            normalize_problem_row(
                {
                    ...
                    "status": "OPEN",
                    "why_it_failed": {
                        "oc_score_materialization": "No target-hidden OC prediction score is materialized for this comparator gap.",
                        "oc_model_residual": "The OC model residual is absent from the strict evidence pack.",
                        "incumbent_comparator_residual": "The incumbent comparator residual is absent from the strict evidence pack.",
                        "material_margin": "The OC residual does not materially beat the preregistered comparator residual.",
                        "fail_closed_status_clear": "The scoring artifact still carries a fail-closed status token.",
                        "strict_pack_pass": "The strict evidence pack has not reached PASS.",
                    }.get(component_id, "A domain-model repair predicate remains open."),
                    "repair_strategy": {
                        "oc_score_materialization": "Bind an executable OC model formula to this target-hidden benchmark and write the score row before target unsealing.",
                        "oc_model_residual": "Compute the OC residual from the locked target and OC prediction under the declared residual rule.",
                        "incumbent_comparator_residual": "Compute the incumbent comparator residual on the same locked target and metric.",
                        "material_margin": "If OC does not beat the comparator, keep the gap fail-closed; otherwise record the source-bound residuals and materiality proof.",
                        "fail_closed_status_clear": "Remove fail-closed status only by producing a PASS strict evidence pack with source-bound residuals.",
                        "strict_pack_pass": "Regenerate the strict evidence pack after all source, score, comparator, uncertainty, falsifier, and replay predicates pass.",
                    }.get(component_id, "Repair the named model/comparator component from source-bound evidence."),
                    "required_capability": "Research/DomainModelComparatorRepair",
                    "execution_command": [sys.executable, "tools/oc133_toe_closure_factory.py", "--execute-comparator-domain-model-repair", comparator_domain_model_repair_id(gap_id), "--write"],
                    "pass_predicate": "The downstream oc_prediction_scoring_row artifact becomes PASS without weakening materiality or replay predicates.",
                    "validator_binding": f"comparator_gap::{gap_id}::domain_model_repair::{component_id}",
                    "next_escalation": "Implement the exact model/comparator source computation for this component; if negative, preserve fail-closed science.",
                    "no_fake_closure_policy": "Component rows are repair obligations only and cannot promote broad superiority.",
                },
                {},
            )
        )
...
def build_comparator_domain_model_repair_backlog(root: Path) -> dict[str, Any]:
    generated_at = stable_generated_at(root, comparator_domain_model_repair_backlog_rel())
    rows: list[dict[str, Any]] = []
    for implementation_row in comparator_domain_scorer_implementation_rows(root):
        if not isinstance(implementation_row, dict):
            continue
        gap_id = str(implementation_row.get("gap_id") or "")
        implementation_id = str(implementation_row.get("implementation_id") or "")
        if not gap_id or not implementation_id:
            continue
        report = comparator_domain_scorer_implementation_report(root, implementation_id)
        if report.get("root_cause_class") != "DOMAIN_SCORER_MODEL_OR_COMPARATOR_REPAIR_REQUIRED":
            continue
        repair_id = comparator_domain_model_repair_id(gap_id)
        if comparator_domain_model_repair_completed(root, repair_id):
            continue
        rows.append(
            normalize_problem_row(
                {
                    "repair_id": repair_id,
                    "repair_work_order_id": repair_id,
                    "lane_id": "MODERN_SCIENCE_COMPARATOR_SUPERIORITY",
                    "gap_id": gap_id,
                    "domain_class_id": implementation_row.get("domain_class_id") or report.get("domain_class_id"),
                    "phenomenon_class_id": implementation_row.get("phenomenon_class_id") or report.get("phenomenon_class_id"),
                    "status": "OPEN",
                    "source_domain_scorer_implementation_ref": report.get("artifact_ref"),
                    "source_domain_scorer_implementation_status": report.get("status"),
                    "source_domain_scorer_root_cause": report.get("root_cause_class"),
                    "scoring_artifact_ref": report.get("scoring_artifact_ref"),
                    "scoring_artifact_status": report.get("scoring_artifact_status"),
                    "missing_model_repair_elements": report.get("missing_domain_scorer_elements", []),
                    "why_it_failed": "The exact domain scorer now exists and runs, but the OC/comparator residual/materiality evidence remains fail-closed.",
                    "repair_strategy": "Repair the domain-local OC model, incumbent comparator scoring, residual computation, uncertainty/materiality, or claim scope from source-bound evidence.",
                    "required_capability": "Research/DomainModelComparatorRepair",
                    "execution_command": [sys.executable, "tools/oc133_toe_closure_factory.py", "--execute-comparator-domain-model-repair", repair_id, "--write"],
                    "pass_predicate": "Downstream oc_prediction_scoring_row and replay predicates pass with source-bound OC/comparator residuals.",
                    "validator_binding": f"comparator_gap::{gap_id}::domain_model_repair",
                    "next_escalation": "Generate component-level model/comparator obligations and keep the gap fail-closed until real residuals pass.",
                    "no_fake_closure_policy": "Domain-model repair rows cannot close broad superiority unless the downstream scoring artifact itself passes.",
                },
                {},
            )
        )
    payload = {
        "schema_id": "OC133_MODERN_SCIENCE_COMPARATOR_DOMAIN_MODEL_REPAIR_BACKLOG_v1",
        "generated_at": generated_at,
        "lane_id": "MODERN_SCIENCE_COMPARATOR_SUPERIORITY",
        "status": "OPEN" if rows else "PASS",
        "domain_model_repair_total": len(rows),
        "open_domain_model_repair_total": sum(1 for row in rows if row.get("status") != "PASS"),
        "execution_command": [sys.executable, "tools/oc133_toe_closure_factory.py", "--compile-comparator-domain-model-repair-backlog", "--write"],
        "why_it_failed": "Domain scorer executions have reached the model/comparator repair frontier." if rows else "No domain model/comparator repair backlog remains.",
        "repair_strategy": "Convert each scorer-ran-but-open row into component-level model/comparator repair obligations.",
        "required_capability": "Research/DomainModelComparatorRepair",
        "pass_predicate": "Every domain-model repair row is PASS through downstream scoring artifacts.",
        "next_escalation": "Execute the first domain-model repair row and inspect component obligations.",
        "no_fake_closure_policy": "Backlog rows are internal repair obligations, not superiority evidence.",
        "rows": rows,
    }
    payload["artifact_ref"] = rel(root, root / comparator_domain_model_repair_backlog_rel())
    payload["artifact_hash"] = artifact_hash(payload)
    write_json_artifact(root, comparator_domain_model_repair_backlog_rel(), payload)
    return payload


def build_comparator_domain_model_repair_execution(root: Path, repair_id: str) -> dict[str, Any]:
    rows_by_id = comparator_domain_model_repair_rows_by_id(root)
    row = rows_by_id.get(repair_id)
    if not row:
        build_comparator_domain_model_repair_backlog(root)
        rows_by_id = comparator_domain_model_repair_rows_by_id(root)
        row = rows_by_id.get(repair_id)
    generated_at = stable_generated_at(root, comparator_domain_model_repair_rel(repair_id))
    if not row:
        payload = normalize_problem_row(
            {
                "schema_id": "OC133_MODERN_SCIENCE_COMPARATOR_DOMAIN_MODEL_REPAIR_EXECUTION_v1",
                "generated_at": generated_at,
                "repair_id": repair_id,
                "lane_id": "MODERN_SCIENCE_COMPARATOR_SUPERIORITY",
                "status": "FAIL_CLOSED",
                "root_cause_class": "DOMAIN_MODEL_REPAIR_BACKLOG_ROW_MISSING",
                "why_it_failed": "No domain-model repair backlog row exists for this id.",
                "repair_strategy": "Recompile the domain-model repair backlog from current domain-scorer implementation reports.",
                "required_capability": "Research/DomainModelComparatorRepair",
                "execution_command": [sys.executable, "tools/oc133_toe_closure_factory.py", "--execute-comparator-domain-model-repair", repair_id, "--write"],
                "pass_predicate": "Backlog row exists and its downstream scoring artifact passes.",
                "validator_binding": f"comparator_domain_model_repair::{repair_id}",
                "next_escalation": "Run --compile-comparator-domain-model-repair-backlog --write.",
                "no_fake_closure_policy": "Missing repair rows cannot close broad superiority.",
            },
            {},
        )
        payload["artifact_ref"] = rel(root, root / comparator_domain_model_repair_rel(repair_id))
        payload["artifact_hash"] = artifact_hash(payload)
        write_json_artifact(root, comparator_domain_model_repair_rel(repair_id), payload)
        return payload
    gap_id = str(row.get("gap_id") or "")
    domain_class_id = str(row.get("domain_class_id") or "")
    phenomenon_class_id = str(row.get("phenomenon_class_id") or "")
    evidence_pack = build_comparator_generic_evidence_pack(root, gap_id) if gap_id else {}
    scoring_artifact = build_comparator_gap_research_artifact(root, gap_id, "oc_prediction_scoring_row") if gap_id else {}
    replay_artifact = build_comparator_gap_research_artifact(root, gap_id, "replay_record") if gap_id else {}
    component_rows = comparator_domain_model_repair_component_rows(
        gap_id,
        domain_class_id,
        phenomenon_class_id,
        scoring_artifact,
    ...
    if scoring_artifact.get("status") == "PASS" and replay_artifact.get("status") == "PASS":
        status = "PASS"
        root_cause = "DOMAIN_MODEL_REPAIR_CLOSED"
        why = "The downstream scoring and replay artifacts now pass strict source-bound predicates."
    else:
        status = "EVIDENCE_REPAIR_ATTEMPTED_REMAINS_OPEN"
        root_cause = "DOMAIN_MODEL_OR_COMPARATOR_SOURCE_GAP"
        why = "Component-level OC model/comparator/residual/materiality obligations remain open."
    payload = normalize_problem_row(
        {
            "schema_id": "OC133_MODERN_SCIENCE_COMPARATOR_DOMAIN_MODEL_REPAIR_EXECUTION_v1",
            "generated_at": generated_at,
            "repair_id": repair_id,
            "repair_work_order_id": row.get("repair_work_order_id"),
            "lane_id": "MODERN_SCIENCE_COMPARATOR_SUPERIORITY",
            "gap_id": gap_id,
            "domain_class_id": domain_class_id,
            "phenomenon_class_id": phenomenon_class_id,
            "status": status,
            "root_cause_class": root_cause,
            "source_domain_scorer_implementation_ref": row.get("source_domain_scorer_implementation_ref"),
            "evidence_pack_ref": evidence_pack.get("artifact_ref"),
            "evidence_pack_status": evidence_pack.get("pack_status") or evidence_pack.get("status"),
            "scoring_artifact_ref": scoring_artifact.get("artifact_ref"),
            "scoring_artifact_status": scoring_artifact.get("status"),
            "replay_artifact_ref": replay_artifact.get("artifact_ref"),
            "replay_artifact_status": replay_artifact.get("status"),
            "component_open_total": len(component_rows),
            "component_rows": component_rows,
            "why_it_failed": why,
            "repair_strategy": "Implement the component rows by binding actual OC predictions, incumbent comparator predictions, residuals, uncertainty, and replay from source-bound data.",
            "required_capability": "Research/DomainModelComparatorRepair",
            "execution_command": [sys.executable, "tools/oc133_toe_closure_factory.py", "--execute-comparator-domain-model-repair", repair_id, "--write"],
            "pass_predicate": "oc_prediction_scoring_row == PASS and replay_record == PASS without fail-closed statuses.",
            "validator_binding": row.get("validator_binding") or f"comparator_gap::{gap_id}::domain_model_repair",
            "next_escalation": "Execute or implement the first component row; if OC is not superior, keep broad superiority blocked.",
            "no_fake_closure_policy": "This repair report cannot close broad superiority unless downstream scoring and replay artifacts pass.",
        },
        {},
    )
    payload["artifact_ref"] = rel(root, root / comparator_domain_model_repair_rel(repair_id))
...
def execute_comparator_domain_model_repair_batch(root: Path, limit: int = 0) -> dict[str, Any]:
    backlog = build_comparator_domain_model_repair_backlog(root)
    rows = [row for row in backlog.get("rows", []) or [] if isinstance(row, dict)]
    executed = []
    for row in rows:
        repair_id = str(row.get("repair_id") or "")
        if not repair_id:
            continue
        payload = build_comparator_domain_model_repair_execution(root, repair_id)
        executed.append(
            {
                "repair_id": repair_id,
                "gap_id": row.get("gap_id"),
                ...
                "root_cause_class": payload.get("root_cause_class"),
                "scoring_artifact_status": payload.get("scoring_artifact_status"),
                "component_open_total": payload.get("component_open_total"),
                "artifact_ref": payload.get("artifact_ref"),
            }
        )
        if limit > 0 and len(executed) >= limit:
            break
    refreshed = build_comparator_domain_model_repair_backlog(root)
    payload = {
        "schema_id": "OC133_MODERN_SCIENCE_COMPARATOR_DOMAIN_MODEL_REPAIR_BATCH_v1",
        "generated_at": utc_now(),
        "status": "PASS",
        "requested_limit": limit,
        "executed_total": len(executed),
        "backlog_ref": refreshed.get("artifact_ref"),
        "backlog_open_domain_model_repair_total": refreshed.get("open_domain_model_repair_total"),
        "rows": executed,
        "no_fake_closure_policy": "Batch execution never promotes broad superiority; it only creates exact model/comparator repair obligations.",
    }
    payload["artifact_hash"] = artifact_hash(payload)
    return payload


def comparator_domain_model_component_rows(root: Path) -> list[dict[str, Any]]:
    payload = read_json(root / comparator_domain_model_component_backlog_rel())
    rows = payload.get("rows") or []
    return [row for row in rows if isinstance(row, dict)]


def comparator_domain_model_component_rows_by_id(root: Path) -> dict[str, dict[str, Any]]:
    return {
        str(row.get("component_work_order_id")): row
        for row in comparator_domain_model_component_rows(root)
        if row.get("component_work_order_id")
    }


def comparator_domain_model_component_report(root: Path, component_work_order_id: str) -> dict[str, Any]:
    payload = read_json(root / comparator_domain_model_component_rel(component_work_order_id))
    return payload if payload.get("component_work_order_id") == component_work_order_id else {}


def comparator_domain_model_component_completed(root: Path, component_work_order_id: str) -> bool:
    payload = comparator_domain_model_component_report(root, component_work_order_id)
    return payload.get("status") == "PASS" and payload.get("scoring_artifact_status") == "PASS"


def build_comparator_domain_model_component_backlog(root: Path) -> dict[str, Any]:
    generated_at = stable_generated_at(root, comparator_domain_model_component_backlog_rel())
    rows: list[dict[str, Any]] = []
    model_backlog = build_comparator_domain_model_repair_backlog(root)
    for model_row in model_backlog.get("rows", []) or []:
        if not isinstance(model_row, dict):
            continue
        repair_id = str(model_row.get("repair_id") or "")
        repair_report = comparator_domain_model_repair_report(root, repair_id) or build_comparator_domain_model_repair_execution(root, repair_id)
        for component in repair_report.get("component_rows", []) or []:
            if not isinstance(component, dict) or component.get("status") == "PASS":
                continue
            gap_id = str(component.get("gap_id") or model_row.get("gap_id") or "")
            component_id = str(component.get("component_id") or "")
            if not gap_id or not component_id:
                continue
            component_work_order_id = comparator_domain_model_component_id(gap_id, component_id)
            if comparator_domain_model_component_completed(root, component_work_order_id):
                continue
            rows.append(
                normalize_problem_row(
                    {
                        ...
                        "lane_id": "MODERN_SCIENCE_COMPARATOR_SUPERIORITY",
                        "gap_id": gap_id,
                        "domain_class_id": component.get("domain_class_id") or model_row.get("domain_class_id"),
                        "phenomenon_class_id": component.get("phenomenon_class_id") or model_row.get("phenomenon_class_id"),
                        "component_id": component_id,
                        "status": "OPEN",
                        "source_domain_model_repair_ref": repair_report.get("artifact_ref"),
                        "why_it_failed": component.get("why_it_failed"),
                        "repair_strategy": component.get("repair_strategy"),
                        "required_capability": component.get("required_capability") or "Research/DomainModelComparatorRepair",
                        "execution_command": [sys.executable, "tools/oc133_toe_closure_factory.py", "--execute-comparator-domain-model-component", component_work_order_id, "--write"],
                        "pass_predicate": component.get("pass_predicate"),
                        "validator_binding": component.get("validator_binding") or f"comparator_gap::{gap_id}::domain_model_component::{component_id}",
                        "next_escalation": "Implement this exact model/comparator component in the domain scorer or preserve the negative result as broad-superiority blocker.",
                        "no_fake_closure_policy": "Component work orders are not evidence and cannot close broad superiority without downstream scoring PASS.",
                    },
                    {},
                )
            )
    payload = {
        "schema_id": "OC133_MODERN_SCIENCE_COMPARATOR_DOMAIN_MODEL_COMPONENT_BACKLOG_v1",
        "generated_at": generated_at,
        "lane_id": "MODERN_SCIENCE_COMPARATOR_SUPERIORITY",
        "status": "OPEN" if rows else "PASS",
        "domain_model_component_total": len(rows),
        "open_domain_model_component_total": sum(1 for row in rows if row.get("status") != "PASS"),
        "domain_model_repair_backlog_ref": model_backlog.get("artifact_ref"),
        "execution_command": [sys.executable, "tools/oc133_toe_closure_factory.py", "--compile-comparator-domain-model-component-backlog", "--write"],
        "why_it_failed": "Domain-model repair reports contain open component obligations." if rows else "No component obligations remain.",
        "repair_strategy": "Execute the exact component rows: score materialization, OC residual, comparator residual, materiality, fail-closed clearing, and strict-pack pass.",
        "required_capability": "Research/DomainModelComparatorRepair",
        "pass_predicate": "Every component row closes through downstream source-bound scoring and replay artifacts.",
        "next_escalation": "Implement the first component work order in the domain scorer/model code.",
        "no_fake_closure_policy": "Component backlog never promotes broad superiority.",
        "rows": rows,
    }
    payload["artifact_ref"] = rel(root, root / comparator_domain_model_component_backlog_rel())
    payload["artifact_hash"] = artifact_hash(payload)
    write_json_artifact(root, comparator_domain_model_component_backlog_rel(), payload)
    return payload


def build_comparator_domain_model_component_execution(root: Path, component_work_order_id: str) -> dict[str, Any]:
    rows_by_id = comparator_domain_model_component_rows_by_id(root)
    row = rows_by_id.get(component_work_order_id)
    if not row:
        build_comparator_domain_model_component_backlog(root)
        rows_by_id = comparator_domain_model_component_rows_by_id(root)
        row = rows_by_id.get(component_work_order_id)
    generated_at = stable_generated_at(root, comparator_domain_model_component_rel(component_work_order_id))
    if not row:
        payload = normalize_problem_row(
            {
                "schema_id": "OC133_MODERN_SCIENCE_COMPARATOR_DOMAIN_MODEL_COMPONENT_EXECUTION_v1",
                "generated_at": generated_at,
                "component_work_order_id": component_work_order_id,
                "lane_id": "MODERN_SCIENCE_COMPARATOR_SUPERIORITY",
                ...
                "execution_command": [sys.executable, "tools/oc133_toe_closure_factory.py", "--execute-comparator-domain-model-component", component_work_order_id, "--write"],
                "pass_predicate": "Backlog row exists and its downstream scoring artifact passes.",
                "validator_binding": f"comparator_domain_model_component::{component_work_order_id}",
                "next_escalation": "Run --compile-comparator-domain-model-component-backlog --write.",
                "no_fake_closure_policy": "Missing component rows cannot close broad superiority.",
            },
            {},
        )
        payload["artifact_ref"] = rel(root, root / comparator_domain_model_component_rel(component_work_order_id))
        payload["artifact_hash"] = artifact_hash(payload)
        write_json_artifact(root, comparator_domain_model_component_rel(component_work_order_id), payload)
        return payload
    gap_id = str(row.get("gap_id") or "")
    component_id = str(row.get("component_id") or "")
    scoring_artifact = build_comparator_gap_research_artifact(root, gap_id, "oc_prediction_scoring_row") if gap_id else {}
    replay_artifact = build_comparator_gap_research_artifact(root, gap_id, "replay_record") if gap_id else {}
    downstream_pass = scoring_artifact.get("status") == "PASS" and replay_artifact.get("status") == "PASS"
    root_cause_by_component = {
        "oc_score_materialization": "OC_SCORE_FUNCTION_NOT_IMPLEMENTED",
        "oc_model_residual": "OC_RESIDUAL_COMPUTATION_NOT_MATERIALIZED",
        "incumbent_comparator_residual": "INCUMBENT_COMPARATOR_RESIDUAL_NOT_MATERIALIZED",
        "material_margin": "MATERIAL_SUPERIORITY_MARGIN_NOT_PROVEN",
        "fail_closed_status_clear": "FAIL_CLOSED_EVIDENCE_STATUS_STILL_PRESENT",
        "strict_pack_pass": "STRICT_EVIDENCE_PACK_NOT_PASSING",
    }
    payload = normalize_problem_row(
        {
            "schema_id": "OC133_MODERN_SCIENCE_COMPARATOR_DOMAIN_MODEL_COMPONENT_EXECUTION_v1",
            ...
            "root_cause_class": "DOMAIN_MODEL_COMPONENT_CLOSED" if downstream_pass else root_cause_by_component.get(component_id, "DOMAIN_MODEL_COMPONENT_EXECUTOR_REQUIRED"),
            "source_domain_model_repair_ref": row.get("source_domain_model_repair_ref"),
            "scoring_artifact_ref": scoring_artifact.get("artifact_ref"),
            "scoring_artifact_status": scoring_artifact.get("status"),
            "replay_artifact_ref": replay_artifact.get("artifact_ref"),
            "replay_artifact_status": replay_artifact.get("status"),
            "implementation_blueprint": {
                ...
                "domain_class_id": row.get("domain_class_id"),
                "phenomenon_class_id": row.get("phenomenon_class_id"),
                "required_code_surface": "domain scorer/model implementation for the named domain and phenomenon",
                "required_output": "source-bound OC prediction, incumbent comparator prediction, residuals, materiality, and replay evidence as applicable",
            },
            "why_it_failed": "Component remains open because downstream scoring/replay artifacts are not PASS." if not downstream_pass else "Component closed through downstream scoring/replay PASS.",
            "repair_strategy": row.get("repair_strategy") or "Implement the named component in source-bound domain scorer/model code.",
            "required_capability": row.get("required_capability") or "Research/DomainModelComparatorRepair",
            "execution_command": [sys.executable, "tools/oc133_toe_closure_factory.py", "--execute-comparator-domain-model-component", component_work_order_id, "--write"],
            "pass_predicate": row.get("pass_predicate"),
            "validator_binding": row.get("validator_binding") or f"comparator_gap::{gap_id}::domain_model_component::{component_id}",
            "next_escalation": "Create or edit the domain-specific model/comparator scorer; if OC remains worse, preserve fail-closed broad superiority.",
            "no_fake_closure_policy": "This component report cannot close broad superiority without downstream scoring and replay PASS.",
        },
        {},
    )
    payload["artifact_ref"] = rel(root, root / comparator_domain_model_component_rel(component_work_order_id))
...
def execute_comparator_domain_model_component_batch(root: Path, limit: int = 0) -> dict[str, Any]:
    backlog = build_comparator_domain_model_component_backlog(root)
    rows = [row for row in backlog.get("rows", []) or [] if isinstance(row, dict)]
    executed = []
    for row in rows:
        component_work_order_id = str(row.get("component_work_order_id") or "")
        if not component_work_order_id:
            continue
        payload = build_comparator_domain_model_component_execution(root, component_work_order_id)
        executed.append(
            {
                "component_work_order_id": component_work_order_id,
                "gap_id": row.get("gap_id"),
        ...
        if limit > 0 and len(executed) >= limit:
            break
    refreshed = build_comparator_domain_model_component_backlog(root)
    payload = {
        "schema_id": "OC133_MODERN_SCIENCE_COMPARATOR_DOMAIN_MODEL_COMPONENT_BATCH_v1",
        "generated_at": utc_now(),
        "status": "PASS",
        "requested_limit": limit,
        "executed_total": len(executed),
        "backlog_ref": refreshed.get("artifact_ref"),
        "backlog_open_domain_model_component_total": refreshed.get("open_domain_model_component_total"),
        "rows": executed,
        "no_fake_closure_policy": "Batch execution never promotes broad superiority; it only creates exact component-level implementation obligations.",
    }
    payload["artifact_hash"] = artifact_hash(payload)
    return payload


def comparator_domain_model_component_implementation_rows(root: Path) -> list[dict[str, Any]]:
    payload = read_json(root / comparator_domain_model_component_implementation_backlog_rel())
    rows = payload.get("rows") or []
    return [row for row in rows if isinstance(row, dict)]


def comparator_domain_model_component_implementation_rows_by_id(root: Path) -> dict[str, dict[str, Any]]:
    return {
        str(row.get("implementation_id")): row
        for row in comparator_domain_model_component_implementation_rows(root)
        if row.get("implementation_id")
    }


def comparator_domain_model_component_implementation_report(root: Path, implementation_id: str) -> dict[str, Any]:
    payload = read_json(root / comparator_domain_model_component_implementation_rel(implementation_id))
    return payload if payload.get("implementation_id") == implementation_id else {}


def comparator_domain_model_component_implementation_completed(root: Path, implementation_id: str) -> bool:
    payload = comparator_domain_model_component_implementation_report(root, implementation_id)
    return (
        payload.get("status") == "PASS"
        and payload.get("component_report_status") == "PASS"
        and payload.get("scoring_artifact_status") == "PASS"
    )


def build_comparator_domain_model_component_implementation_backlog(root: Path) -> dict[str, Any]:
    generated_at = stable_generated_at(root, comparator_domain_model_component_implementation_backlog_rel())
    component_backlog = build_comparator_domain_model_component_backlog(root)
    rows: list[dict[str, Any]] = []
    for component_row in component_backlog.get("rows", []) or []:
        if not isinstance(component_row, dict):
            continue
        component_work_order_id = str(component_row.get("component_work_order_id") or "")
        gap_id = str(component_row.get("gap_id") or "")
        component_id = str(component_row.get("component_id") or "")
        if not component_work_order_id or not gap_id or not component_id:
            continue
        implementation_id = comparator_domain_model_component_implementation_id(component_work_order_id)
        if comparator_domain_model_component_implementation_completed(root, implementation_id):
            continue
        domain_class_id = str(component_row.get("domain_class_id") or "")
        phenomenon_class_id = str(component_row.get("phenomenon_class_id") or "")
        concrete_commands = comparator_domain_model_component_commands(
            domain_class_id,
            phenomenon_class_id,
            component_id,
            gap_id,
            ...
            normalize_problem_row(
                {
                    "implementation_id": implementation_id,
                    "component_work_order_id": component_work_order_id,
                    "repair_id": component_row.get("repair_id"),
                    "lane_id": "MODERN_SCIENCE_COMPARATOR_SUPERIORITY",
                    "gap_id": gap_id,
                    "domain_class_id": domain_class_id,
                    "phenomenon_class_id": phenomenon_class_id,
                    "component_id": component_id,
                    "status": "OPEN",
                    "source_component_work_order_ref": component_row.get("source_domain_model_repair_ref"),
                    "component_report_ref": rel(root, root / comparator_domain_model_component_rel(component_work_order_id)),
                    "component_implementation_report_ref": rel(root, root / comparator_domain_model_component_implementation_rel(implementation_id)),
                    "concrete_command_available": bool(concrete_commands),
                    "concrete_governed_commands": concrete_commands,
                    "why_it_failed": component_row.get("why_it_failed") or "A domain model/comparator component is still open.",
                    "repair_strategy": "Run the concrete domain scorer/model commands that can materialize this component, then regenerate scoring, replay, and strict evidence artifacts.",
                    "required_capability": component_row.get("required_capability") or "Research/DomainModelComparatorRepair",
                    "execution_command": [sys.executable, "tools/oc133_toe_closure_factory.py", "--execute-comparator-domain-model-component-implementation", implementation_id, "--write"],
                    "pass_predicate": "Concrete governed commands run and the downstream component, scoring artifact, and replay artifact become PASS.",
                    "validator_binding": component_row.get("validator_binding") or f"comparator_gap::{gap_id}::domain_model_component_implementation::{component_id}",
                    "next_escalation": "If commands are absent, implement the exact domain scorer/model surface. If commands run and OC remains worse, preserve the negative broad-superiority blocker.",
                    "no_fake_closure_policy": "Component implementation rows cannot close broad superiority unless downstream source-bound scoring and replay artifacts pass.",
                },
                {},
            )
        )
    payload = {
        "schema_id": "OC133_MODERN_SCIENCE_COMPARATOR_DOMAIN_MODEL_COMPONENT_IMPLEMENTATION_BACKLOG_v1",
        "generated_at": generated_at,
        "lane_id": "MODERN_SCIENCE_COMPARATOR_SUPERIORITY",
        "status": "OPEN" if rows else "PASS",
        "domain_model_component_implementation_total": len(rows),
        "open_domain_model_component_implementation_total": sum(1 for row in rows if row.get("status") != "PASS"),
        "component_backlog_ref": component_backlog.get("artifact_ref"),
        "execution_command": [sys.executable, "tools/oc133_toe_closure_factory.py", "--compile-comparator-domain-model-component-implementation-backlog", "--write"],
        "why_it_failed": "Open component obligations still need concrete domain scorer/model implementation commands." if rows else "No component implementation obligations remain.",
        "repair_strategy": "Compile every open component into a command-bound implementation row before treating it as exhausted.",
        "required_capability": "Research/DomainModelComparatorRepair",
        "pass_predicate": "Every component implementation row either closes through PASS scoring/replay or remains an explicit source-bound blocker.",
        "next_escalation": "Execute the first component implementation row and inspect command trace plus downstream scoring artifacts.",
        "no_fake_closure_policy": "Backlog rows are internal implementation obligations and cannot promote modern_science_comparator_superiority.",
        "rows": rows,
    }
    payload["artifact_ref"] = rel(root, root / comparator_domain_model_component_implementation_backlog_rel())
    payload["artifact_hash"] = artifact_hash(payload)
    write_json_artifact(root, comparator_domain_model_component_implementation_backlog_rel(), payload)
    return payload


def build_comparator_domain_model_component_implementation_execution(
    root: Path,
    implementation_id: str,
    timeout: int = 900,
    command_cache: dict[tuple[str, ...], dict[str, Any]] | None = None,
) -> dict[str, Any]:
    rows_by_id = comparator_domain_model_component_implementation_rows_by_id(root)
    row = rows_by_id.get(implementation_id)
    if not row:
        build_comparator_domain_model_component_implementation_backlog(root)
        rows_by_id = comparator_domain_model_component_implementation_rows_by_id(root)
        row = rows_by_id.get(implementation_id)
    generated_at = stable_generated_at(root, comparator_domain_model_component_implementation_rel(implementation_id))
    if not row:
        payload = normalize_problem_row(
            {
                "schema_id": "OC133_MODERN_SCIENCE_COMPARATOR_DOMAIN_MODEL_COMPONENT_IMPLEMENTATION_EXECUTION_v1",
                "generated_at": generated_at,
                "implementation_id": implementation_id,
                "lane_id": "MODERN_SCIENCE_COMPARATOR_SUPERIORITY",
                "status": "FAIL_CLOSED",
                ...
                "required_capability": "Research/DomainModelComparatorRepair",
                "execution_command": [sys.executable, "tools/oc133_toe_closure_factory.py", "--execute-comparator-domain-model-component-implementation", implementation_id, "--write"],
                "pass_predicate": "Backlog row exists and downstream scoring/replay predicates pass.",
                "validator_binding": f"comparator_domain_model_component_implementation::{implementation_id}",
                "next_escalation": "Run --compile-comparator-domain-model-component-implementation-backlog --write.",
                "no_fake_closure_policy": "Missing implementation rows cannot close broad superiority.",
            },
            {},
        )
        payload["artifact_ref"] = rel(root, root / comparator_domain_model_component_implementation_rel(implementation_id))
        payload["artifact_hash"] = artifact_hash(payload)
        write_json_artifact(root, comparator_domain_model_component_implementation_rel(implementation_id), payload)
        return payload

    gap_id = str(row.get("gap_id") or "")
    component_work_order_id = str(row.get("component_work_order_id") or "")
    commands = [list(command) for command in row.get("concrete_governed_commands", []) if isinstance(command, list)]
    command_results: list[dict[str, Any]] = []
    for command in commands:
        key = tuple(str(part) for part in command)
        if command_cache is not None and key in command_cache:
            cached = dict(command_cache[key])
            cached["cache_hit"] = True
            command_results.append(cached)
            continue
        result = safe_run_command(root, command, timeout)
        if command_cache is not None:
            command_cache[key] = dict(result)
        command_results.append(result)
    command_pass = bool(commands) and all(result.get("returncode") == 0 for result in command_results)
    # Refresh the source-bound comparator evidence surface after any concrete
    # command run. These builders remain fail-closed if the scorer did not
    # actually materialize a better OC residual and replay evidence.
    evidence_pack = build_comparator_generic_evidence_pack(root, gap_id) if gap_id else {}
    scoring_artifact = build_comparator_gap_research_artifact(root, gap_id, "oc_prediction_scoring_row") if gap_id else {}
    replay_artifact = build_comparator_gap_research_artifact(root, gap_id, "replay_record") if gap_id else {}
    component_report = build_comparator_domain_model_component_execution(root, component_work_order_id) if component_work_order_id else {}
    downstream_pass = (
        component_report.get("status") == "PASS"
        and scoring_artifact.get("status") == "PASS"
        and replay_artifact.get("status") == "PASS"
    )
    if downstream_pass:
        status = "PASS"
        root_cause = "DOMAIN_MODEL_COMPONENT_IMPLEMENTATION_CLOSED"
        why = "Concrete component commands ran and downstream scoring/replay artifacts now pass."
    elif not commands:
        status = "CAPABILITY_DEVELOPMENT_REQUIRED"
        root_cause = "DOMAIN_MODEL_COMPONENT_COMMAND_MISSING"
        why = "No concrete governed command is bound for this domain/model component."
    elif command_pass:
        status = "EVIDENCE_REPAIR_ATTEMPTED_REMAINS_OPEN"
        root_cause = "DOMAIN_MODEL_COMPONENT_COMMAND_RAN_BUT_EVIDENCE_OPEN"
        why = "Concrete commands ran, but the downstream scoring/replay evidence remains fail-closed or negative."
    else:
        status = "FAIL_CLOSED"
        root_cause = "DOMAIN_MODEL_COMPONENT_COMMAND_FAILED"
        why = "At least one concrete component implementation command failed."
    payload = normalize_problem_row(
        {
            "schema_id": "OC133_MODERN_SCIENCE_COMPARATOR_DOMAIN_MODEL_COMPONENT_IMPLEMENTATION_EXECUTION_v1",
            "generated_at": generated_at,
            ...
            "status": status,
            "root_cause_class": root_cause,
            "implementation_command_class": "CONCRETE_DOMAIN_MODEL_COMPONENT_EXECUTOR" if commands else "MISSING_DOMAIN_MODEL_COMPONENT_EXECUTOR",
            "concrete_governed_commands": commands,
            "command_result_total": len(command_results),
            "command_pass": command_pass,
            "command_results": command_results,
            "evidence_pack_ref": evidence_pack.get("artifact_ref"),
            "evidence_pack_status": evidence_pack.get("pack_status") or evidence_pack.get("status"),
            "component_report_ref": component_report.get("artifact_ref"),
            "component_report_status": component_report.get("status"),
            "component_report_root_cause": component_report.get("root_cause_class"),
            "scoring_artifact_ref": scoring_artifact.get("artifact_ref"),
            "scoring_artifact_status": scoring_artifact.get("status"),
            "replay_artifact_ref": replay_artifact.get("artifact_ref"),
            "replay_artifact_status": replay_artifact.get("status"),
            "why_it_failed": why,
            "repair_strategy": row.get("repair_strategy") or "Implement the concrete domain scorer/model component and preserve negative science if OC does not beat the comparator.",
            "required_capability": row.get("required_capability") or "Research/DomainModelComparatorRepair",
            "execution_command": [sys.executable, "tools/oc133_toe_closure_factory.py", "--execute-comparator-domain-model-component-implementation", implementation_id, "--write"],
            "pass_predicate": row.get("pass_predicate"),
            "validator_binding": row.get("validator_binding") or f"comparator_domain_model_component_implementation::{implementation_id}",
            "next_escalation": "If commands are missing, implement the domain-specific scorer/model surface. If commands run and remain negative, broaden the source/model research obligation without flipping PASS.",
            "no_fake_closure_policy": "This report cannot close broad superiority without downstream scoring and replay PASS.",
        },
        {},
    )
    payload["artifact_ref"] = rel(root, root / comparator_domain_model_component_implementation_rel(implementation_id))
...
def build_comparator_domain_model_component_implementation_registry(root: Path) -> dict[str, Any]:
    backlog = build_comparator_domain_model_component_implementation_backlog(root)
    rows = []
    for row in backlog.get("rows", []) or []:
        if not isinstance(row, dict):
            continue
        implementation_id = str(row.get("implementation_id") or "")
        report = comparator_domain_model_component_implementation_report(root, implementation_id)
        report_status = report.get("status")
        rows.append(
            {
                "implementation_id": implementation_id,
                ...
                "component_id": row.get("component_id"),
                "concrete_command_available": row.get("concrete_command_available") is True,
                "status": report_status if report_status in {"PASS", "CAPABILITY_DEVELOPMENT_REQUIRED", "EVIDENCE_REPAIR_ATTEMPTED_REMAINS_OPEN", "FAIL_CLOSED"} else row.get("status"),
                "implementation_command_class": report.get("implementation_command_class"),
                "component_report_status": report.get("component_report_status"),
                "scoring_artifact_status": report.get("scoring_artifact_status"),
                "replay_artifact_status": report.get("replay_artifact_status"),
                "report_ref": report.get("artifact_ref") or row.get("component_implementation_report_ref"),
                "validator_binding": row.get("validator_binding"),
            }
        )
    payload = {
        "schema_id": "OC133_MODERN_SCIENCE_COMPARATOR_DOMAIN_MODEL_COMPONENT_IMPLEMENTATION_REGISTRY_v1",
        "generated_at": utc_now(),
        "lane_id": "MODERN_SCIENCE_COMPARATOR_SUPERIORITY",
        ...
        "rows": rows,
        "execution_command": [sys.executable, "tools/oc133_toe_closure_factory.py", "--compile-comparator-domain-model-component-implementation-backlog", "--write"],
        "no_fake_closure_policy": "Registry summarizes component implementation only; broad superiority is derived from strict comparator artifacts.",
    }
    payload["artifact_ref"] = rel(root, root / comparator_domain_model_component_implementation_registry_rel())
    payload["artifact_hash"] = artifact_hash(payload)
    write_json_artifact(root, comparator_domain_model_component_implementation_registry_rel(), payload)
    return payload


def execute_comparator_domain_model_component_implementation_batch(
    root: Path,
    timeout: int = 900,
    limit: int = 0,
) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        if not implementation_id:
            continue
        payload = build_comparator_domain_model_component_implementation_execution(
            root,
            implementation_id,
            timeout=timeout,
        ...
        executed.append(
            {
                ...
                "status": payload.get("status"),
                "root_cause_class": payload.get("root_cause_class"),
                "component_report_status": payload.get("component_report_status"),
                "scoring_artifact_status": payload.get("scoring_artifact_status"),
                "artifact_ref": payload.get("artifact_ref"),
            }
        )
        if limit > 0 and len(executed) >= limit:
            break
    registry = build_comparator_domain_model_component_implementation_registry(root)
    payload = {
        "schema_id": "OC133_MODERN_SCIENCE_COMPARATOR_DOMAIN_MODEL_COMPONENT_IMPLEMENTATION_BATCH_v1",
        "generated_at": utc_now(),
        "status": "PASS",
        "requested_limit": limit,
        "executed_total": len(executed),
        "unique_command_total": len(command_cache),
        "registry_ref": registry.get("artifact_ref"),
        "registry_open_implementation_total": registry.get("open_implementation_total"),
        "rows": executed,
        "no_fake_closure_policy": "Batch execution never promotes broad superiority; it records concrete component implementation attempts and fail-closed outcomes.",
    }
    payload["artifact_hash"] = artifact_hash(payload)
    return payload


def comparator_component_source_obligation_rows(root: Path) -> list[dict[str, Any]]:
    payload = read_json(root / comparator_component_source_obligation_backlog_rel())
    rows = payload.get("rows") or []
    return [row for row in rows if isinstance(row, dict)]


def comparator_component_source_obligation_rows_by_id(root: Path) -> dict[str, dict[str, Any]]:
    return {
        str(row.get("obligation_id")): row
        for row in comparator_component_source_obligation_rows(root)
        if row.get("obligation_id")
    }


def comparator_component_source_obligation_report(root: Path, obligation_id: str) -> dict[str, Any]:
    payload = read_json(root / comparator_component_source_obligation_rel(obligation_id))
    return payload if payload.get("obligation_id") == obligation_id else {}


def comparator_component_source_obligation_completed(root: Path, obligation_id: str) -> bool:
    payload = comparator_component_source_obligation_report(root, obligation_id)
    return (
        payload.get("status") == "PASS"
        and payload.get("scoring_artifact_status") == "PASS"
        and payload.get("replay_artifact_status") == "PASS"
    )


def comparator_component_source_obligation_missing_fields(
    ...
    residuals = summary.get("residuals", {}) if isinstance(summary.get("residuals"), dict) else {}
    fields = {
        "component_report_pass": implementation_report.get("component_report_status") == "PASS",
        "strict_evidence_pack_pass": (evidence_pack.get("pack_status") or evidence_pack.get("status")) == "PASS",
        "scoring_artifact_pass": scoring_artifact.get("status") == "PASS",
        "replay_artifact_pass": replay_artifact.get("status") == "PASS",
        "source_bound": summary.get("source_bound") is True or evidence_pack.get("source_bound") is True,
        "target_hidden": summary.get("target_hidden") is True or evidence_pack.get("target_hidden") is True,
        "score_materialized": summary.get("score_materialized") is True or evidence_pack.get("score_materialized") is True,
        "oc_model_residual": isinstance(residuals.get("model"), (int, float)),
        "incumbent_comparator_residual": isinstance(residuals.get("comparator"), (int, float)),
        "material_margin_met": residuals.get("material_margin_met") is True,
        "fail_closed_status_clear": validation.get("fail_closed_status_present") is False and not str(evidence_pack.get("status") or "").upper().startswith("FAIL"),
    }
    return [key for key, value in fields.items() if value is not True]


def comparator_component_source_next_commands(root: Path, gap_id: str, component_work_order_id: str) -> list[list[str]]:
    commands: list[list[str]] = []
    if gap_id:
        scoring_id = comparator_domain_scoring_id(gap_id)
        scorer_impl_id = comparator_domain_scorer_implementation_id(gap_id)
        repair_id = comparator_domain_model_repair_id(gap_id)
        commands.extend(
            [
                [sys.executable, "tools/oc133_toe_closure_factory.py", "--execute-comparator-domain-scoring", scoring_id, "--write"],
                [sys.executable, "tools/oc133_toe_closure_factory.py", "--execute-comparator-domain-scorer-implementation", scorer_impl_id, "--write"],
                [sys.executable, "tools/oc133_toe_closure_factory.py", "--execute-comparator-domain-model-repair", repair_id, "--write"],
            ]
        )
    if component_work_order_id:
        commands.append(
            [
                sys.executable,
                "tools/oc133_toe_closure_factory.py",
                "--execute-comparator-domain-model-component",
                component_work_order_id,
                "--write",
            ]
        )
    if gap_id:
        commands.append(
            [sys.executable, "tools/oc133_toe_closure_factory.py", "--execute-comparator-generic-evidence-pack", gap_id, "--write"]
        )
    return unique_commands(commands)


def build_comparator_component_source_obligation_backlog(root: Path) -> dict[str, Any]:
    generated_at = stable_generated_at(root, comparator_component_source_obligation_backlog_rel())
    component_registry = build_comparator_domain_model_component_implementation_registry(root)
    rows: list[dict[str, Any]] = []
    for registry_row in component_registry.get("rows", []) or []:
        if not isinstance(registry_row, dict):
            continue
        implementation_id = str(registry_row.get("implementation_id") or "")
        report = comparator_domain_model_component_implementation_report(root, implementation_id)
        if not report or report.get("status") == "PASS":
            continue
        if report.get("root_cause_class") not in {
            "DOMAIN_MODEL_COMPONENT_COMMAND_RAN_BUT_EVIDENCE_OPEN",
            "DOMAIN_MODEL_COMPONENT_COMMAND_MISSING",
            "DOMAIN_MODEL_COMPONENT_COMMAND_FAILED",
        }:
            continue
        gap_id = str(report.get("gap_id") or registry_row.get("gap_id") or "")
        component_id = str(report.get("component_id") or registry_row.get("component_id") or "")
        component_work_order_id = str(report.get("component_work_order_id") or registry_row.get("component_work_order_id") or "")
        if not gap_id or not component_id or not implementation_id:
            continue
        obligation_id = comparator_component_source_obligation_id(implementation_id, component_id)
        if comparator_component_source_obligation_completed(root, obligation_id):
            continue
        evidence_ref = str(report.get("evidence_pack_ref") or comparator_gap_generated_evidence_pack_rel(gap_id).as_posix())
        scoring_ref = str(report.get("scoring_artifact_ref") or comparator_gap_research_artifact_rel(gap_id, "oc_prediction_scoring_row").as_posix())
        replay_ref = str(report.get("replay_artifact_ref") or comparator_gap_research_artifact_rel(gap_id, "replay_record").as_posix())
        evidence_pack = read_json(root / evidence_ref) if evidence_ref else {}
        scoring_artifact = read_json(root / scoring_ref) if scoring_ref else {}
        replay_artifact = read_json(root / replay_ref) if replay_ref else {}
        queue_row = comparator_lane_queue_rows_by_gap(root).get(gap_id, {})
        executable_spec = queue_row.get("executable_work_order", {}) if isinstance(queue_row.get("executable_work_order"), dict) else {}
        source = executable_spec.get("official_data_source", {}) if isinstance(executable_spec.get("official_data_source"), dict) else {}
        target = executable_spec.get("target_variable", {}) if isinstance(executable_spec.get("target_variable"), dict) else {}
        comparator = executable_spec.get("incumbent_comparator_requirement", {}) if isinstance(executable_spec.get("incumbent_comparator_requirement"), dict) else {}
        residual = executable_spec.get("residual_requirement", {}) if isinstance(executable_spec.get("residual_requirement"), dict) else {}
        missing_fields = comparator_component_source_obligation_missing_fields(report, evidence_pack, scoring_artifact, replay_artifact)
        required_snapshot_ref = source.get("required_local_snapshot_ref")
        required_lock_ref = source.get("required_lock_ref")
        source_snapshot_exists = bool(required_snapshot_ref) and (root / str(required_snapshot_ref)).exists()
        source_lock_exists = bool(required_lock_ref) and (root / str(required_lock_ref)).exists()
        rows.append(
            normalize_problem_row(
                {
                    ...
                    "lane_id": "MODERN_SCIENCE_COMPARATOR_SUPERIORITY",
                    "gap_id": gap_id,
                    "domain_class_id": report.get("domain_class_id") or registry_row.get("domain_class_id"),
                    "phenomenon_class_id": report.get("phenomenon_class_id") or registry_row.get("phenomenon_class_id"),
                    "component_id": component_id,
                    "status": "OPEN",
                    "source_component_implementation_ref": report.get("artifact_ref") or registry_row.get("report_ref"),
                    "source_component_implementation_status": report.get("status"),
                    "source_component_root_cause": report.get("root_cause_class"),
                    "evidence_pack_ref": evidence_ref,
                    "evidence_pack_status": evidence_pack.get("pack_status") or evidence_pack.get("status"),
                    "scoring_artifact_ref": scoring_ref,
                    "scoring_artifact_status": scoring_artifact.get("status"),
                    "replay_artifact_ref": replay_ref,
                    "replay_artifact_status": replay_artifact.get("status"),
                    "missing_source_evidence_fields": missing_fields,
                    "source_plan_id": executable_spec.get("source_plan_id"),
                    "official_data_source": source,
                    "target_variable": target,
                    "incumbent_comparator": comparator,
                    "residual_requirement": residual,
                    "required_local_snapshot_ref": required_snapshot_ref,
                    "required_lock_ref": required_lock_ref,
                    "source_snapshot_exists": source_snapshot_exists,
                    "source_lock_exists": source_lock_exists,
                    "concrete_next_commands": comparator_component_source_next_commands(root, gap_id, component_work_order_id),
                    "why_it_failed": "Concrete component commands executed, but source-bound scoring/replay evidence is still not materialized for this exact component.",
                    "repair_strategy": "Materialize the named missing source/model/comparator/residual/replay fields from governed source data; preserve fail-closed science if OC does not beat the comparator.",
                    "required_capability": "Research/SourceBoundScoringMaterialization",
                    "execution_command": [sys.executable, "tools/oc133_toe_closure_factory.py", "--execute-comparator-component-source-obligation", obligation_id, "--write"],
                    "pass_predicate": "The exact component implementation, strict evidence pack, oc_prediction_scoring_row, and replay_record are all PASS without weakening materiality.",
                    "validator_binding": f"comparator_gap::{gap_id}::component_source_obligation::{component_id}",
                    "next_escalation": "If the same fields remain missing after execution, implement the domain source acquisition/scoring script named by official_data_source and target_variable.",
                    "no_fake_closure_policy": "This obligation cannot promote broad superiority; only downstream PASS evidence artifacts can.",
                },
                {},
            )
        )
    payload = {
        "schema_id": "OC133_MODERN_SCIENCE_COMPARATOR_COMPONENT_SOURCE_OBLIGATION_BACKLOG_v1",
        "generated_at": generated_at,
        "lane_id": "MODERN_SCIENCE_COMPARATOR_SUPERIORITY",
        "status": "OPEN" if rows else "PASS",
        "component_source_obligation_total": len(rows),
        "open_component_source_obligation_total": sum(1 for row in rows if row.get("status") != "PASS"),
        "component_implementation_registry_ref": component_registry.get("artifact_ref"),
        "execution_command": [sys.executable, "tools/oc133_toe_closure_factory.py", "--compile-comparator-component-source-obligation-backlog", "--write"],
        "why_it_failed": "Component implementation reports reached the source-bound scoring materialization frontier." if rows else "No component source obligations remain.",
        "repair_strategy": "Compile every command-ran-but-evidence-open component into a source/model/comparator/residual/replay obligation.",
        "required_capability": "Research/SourceBoundScoringMaterialization",
        "pass_predicate": "Every component source obligation is closed by downstream PASS scoring and replay artifacts.",
        "next_escalation": "Execute exact source obligations; if unchanged, generate domain source acquisition/scoring implementation tasks.",
        "no_fake_closure_policy": "Backlog rows are internal obligations and cannot promote modern_science_comparator_superiority.",
        "rows": rows,
    }
    payload["artifact_ref"] = rel(root, root / comparator_component_source_obligation_backlog_rel())
    payload["artifact_hash"] = artifact_hash(payload)
    write_json_artifact(root, comparator_component_source_obligation_backlog_rel(), payload)
    return payload


def build_comparator_component_source_obligation_execution(
    root: Path,
    obligation_id: str,
    timeout: int = 900,
    command_cache: dict[tuple[str, ...], dict[str, Any]] | None = None,
) -> dict[str, Any]:
    rows_by_id = comparator_component_source_obligation_rows_by_id(root)
    row = rows_by_id.get(obligation_id)
    if not row:
        build_comparator_component_source_obligation_backlog(root)
        rows_by_id = comparator_component_source_obligation_rows_by_id(root)
        row = rows_by_id.get(obligation_id)
    generated_at = stable_generated_at(root, comparator_component_source_obligation_rel(obligation_id))
    if not row:
        payload = normalize_problem_row(
            {
                "schema_id": "OC133_MODERN_SCIENCE_COMPARATOR_COMPONENT_SOURCE_OBLIGATION_EXECUTION_v1",
                "generated_at": generated_at,
                "obligation_id": obligation_id,
                "lane_id": "MODERN_SCIENCE_COMPARATOR_SUPERIORITY",
                "status": "FAIL_CLOSED",
                "root_cause_class": "COMPONENT_SOURCE_OBLIGATION_ROW_MISSING",
                "why_it_failed": "No component source obligation row exists for this id.",
                "repair_strategy": "Recompile component source obligation backlog from current component implementation reports.",
                "required_capability": "Research/SourceBoundScoringMaterialization",
                "execution_command": [sys.executable, "tools/oc133_toe_closure_factory.py", "--execute-comparator-component-source-obligation", obligation_id, "--write"],
                "pass_predicate": "Backlog row exists and downstream scoring/replay predicates pass.",
        ...
        payload["artifact_ref"] = rel(root, root / comparator_component_source_obligation_rel(obligation_id))
        payload["artifact_hash"] = artifact_hash(payload)
        write_json_artifact(root, comparator_component_source_obligation_rel(obligation_id), payload)
        return payload

    gap_id = str(row.get("gap_id") or "")
    implementation_id = str(row.get("implementation_id") or "")
    component_work_order_id = str(row.get("component_work_order_id") or "")
    commands = [list(command) for command in row.get("concrete_next_commands", []) if isinstance(command, list)]
    command_results: list[dict[str, Any]] = []
    for command in commands:
        key = tuple(str(part) for part in command)
        if command_cache is not None and key in command_cache:
        ...
        command_results.append(result)
    command_pass = bool(commands) and all(result.get("returncode") == 0 for result in command_results)
    component_report = build_comparator_domain_model_component_implementation_execution(root, implementation_id, timeout=timeout, command_cache=command_cache) if implementation_id else {}
    evidence_pack = build_comparator_generic_evidence_pack(root, gap_id) if gap_id else {}
    scoring_artifact = build_comparator_gap_research_artifact(root, gap_id, "oc_prediction_scoring_row") if gap_id else {}
    replay_artifact = build_comparator_gap_research_artifact(root, gap_id, "replay_record") if gap_id else {}
    missing_fields = comparator_component_source_obligation_missing_fields(component_report, evidence_pack, scoring_artifact, replay_artifact)
    downstream_pass = (
        component_report.get("status") == "PASS"
        and (evidence_pack.get("pack_status") or evidence_pack.get("status")) == "PASS"
        and scoring_artifact.get("status") == "PASS"
        and replay_artifact.get("status") == "PASS"
    )
    if downstream_pass:
        status = "PASS"
        root_cause = "COMPONENT_SOURCE_OBLIGATION_CLOSED"
        why = "Downstream component, strict evidence, scoring, and replay artifacts now pass."
    elif command_pass:
        status = "SOURCE_BOUND_SCORING_MATERIALIZATION_REMAINS_OPEN"
        root_cause = "SOURCE_BOUND_SCORE_OR_REPLAY_NOT_MATERIALIZED"
        why = "Narrow source/model/comparator commands ran, but required scoring/replay fields remain missing or fail-closed."
    elif commands:
        status = "FAIL_CLOSED"
        root_cause = "COMPONENT_SOURCE_OBLIGATION_COMMAND_FAILED"
        why = "At least one source-bound scoring materialization command failed."
    else:
        status = "CAPABILITY_DEVELOPMENT_REQUIRED"
        root_cause = "COMPONENT_SOURCE_OBLIGATION_COMMAND_MISSING"
        why = "No concrete source-bound materialization command is available for this obligation."
    payload = normalize_problem_row(
        {
            "schema_id": "OC133_MODERN_SCIENCE_COMPARATOR_COMPONENT_SOURCE_OBLIGATION_EXECUTION_v1",
            "generated_at": generated_at,
            ...
            "command_pass": command_pass,
            "command_results": command_results,
            "source_component_implementation_ref": component_report.get("artifact_ref") or row.get("source_component_implementation_ref"),
            "source_component_implementation_status": component_report.get("status"),
            "evidence_pack_ref": evidence_pack.get("artifact_ref") or row.get("evidence_pack_ref"),
            "evidence_pack_status": evidence_pack.get("pack_status") or evidence_pack.get("status"),
            ...
            "why_it_failed": why,
            "repair_strategy": row.get("repair_strategy"),
            "required_capability": row.get("required_capability") or "Research/SourceBoundScoringMaterialization",
            "execution_command": [sys.executable, "tools/oc133_toe_closure_factory.py", "--execute-comparator-component-source-obligation", obligation_id, "--write"],
            "pass_predicate": row.get("pass_predicate"),
            "validator_binding": row.get("validator_binding") or f"comparator_component_source_obligation::{obligation_id}",
            "next_escalation": "Implement the exact source acquisition/scoring script for the missing fields, or preserve the fail-closed comparator result if OC does not beat the incumbent.",
            "no_fake_closure_policy": "This execution can close only after downstream source-bound evidence artifacts pass.",
        },
        {},
    )
    payload["artifact_ref"] = rel(root, root / comparator_component_source_obligation_rel(obligation_id))
...
def build_comparator_component_source_obligation_registry(root: Path) -> dict[str, Any]:
    backlog = build_comparator_component_source_obligation_backlog(root)
    rows = []
    for row in backlog.get("rows", []) or []:
        if not isinstance(row, dict):
            continue
        ...
        rows.append(
            {
                ...
                "phenomenon_class_id": row.get("phenomenon_class_id"),
                "component_id": row.get("component_id"),
                "status": report_status if report_status in {"PASS", "CAPABILITY_DEVELOPMENT_REQUIRED", "SOURCE_BOUND_SCORING_MATERIALIZATION_REMAINS_OPEN", "FAIL_CLOSED"} else row.get("status"),
                "source_component_implementation_status": report.get("source_component_implementation_status"),
                "scoring_artifact_status": report.get("scoring_artifact_status"),
                "replay_artifact_status": report.get("replay_artifact_status"),
                "missing_source_evidence_fields": report.get("missing_source_evidence_fields") or row.get("missing_source_evidence_fields"),
                "report_ref": report.get("artifact_ref") or row.get("component_source_obligation_report_ref"),
                "validator_binding": row.get("validator_binding"),
            }
        )
    payload = {
        "schema_id": "OC133_MODERN_SCIENCE_COMPARATOR_COMPONENT_SOURCE_OBLIGATION_REGISTRY_v1",
        "generated_at": utc_now(),
        "lane_id": "MODERN_SCIENCE_COMPARATOR_SUPERIORITY",
        "status": "PASS" if rows and all(row.get("status") == "PASS" for row in rows) else "OPEN" if rows else "PASS",
        "component_source_obligation_total": len(rows),
        "open_component_source_obligation_total": sum(1 for row in rows if row.get("status") != "PASS"),
        "obligation_total": len(rows),
        "open_obligation_total": sum(1 for row in rows if row.get("status") != "PASS"),
        "rows": rows,
        "execution_command": [sys.executable, "tools/oc133_toe_closure_factory.py", "--compile-comparator-component-source-obligation-backlog", "--write"],
        "no_fake_closure_policy": "Registry summarizes source-bound component obligations only; broad superiority is derived from strict comparator artifacts.",
    }
    payload["artifact_ref"] = rel(root, root / comparator_component_source_obligation_registry_rel())
    payload["artifact_hash"] = artifact_hash(payload)
    write_json_artifact(root, comparator_component_source_obligation_registry_rel(), payload)
    return payload


def execute_comparator_component_source_obligation_batch(
    root: Path,
    timeout: int = 900,
    limit: int = 0,
) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        if not obligation_id:
            continue
        payload = build_comparator_component_source_obligation_execution(
            root,
            obligation_id,
            timeout=timeout,
            command_cache=command_cache,
        ...
        executed.append(
            {
                ...
                "status": payload.get("status"),
                "root_cause_class": payload.get("root_cause_class"),
                "missing_source_evidence_fields": payload.get("missing_source_evidence_fields"),
                "artifact_ref": payload.get("artifact_ref"),
            }
        )
        if limit > 0 and len(executed) >= limit:
            break
    registry = build_comparator_component_source_obligation_registry(root)
    payload = {
        "schema_id": "OC133_MODERN_SCIENCE_COMPARATOR_COMPONENT_SOURCE_OBLIGATION_BATCH_v1",
        "generated_at": utc_now(),
        "status": "PASS",
        "requested_limit": limit,
        "executed_total": len(executed),
...
def build_comparator_source_executor_work_order(root: Path, gap_id: str, subartifact_id: str) -> dict[str, Any]:
    ...
    payload = normalize_problem_row(
        {
            "schema_id": "OC133_MODERN_SCIENCE_COMPARATOR_SOURCE_EXECUTOR_WORK_ORDER_v1",
            "generated_at": generated_at,
            "lane_id": "MODERN_SCIENCE_COMPARATOR_SUPERIORITY",
            "gap_id": gap_id,
            "scoring_subartifact_id": subartifact_id,
            ...
            "execution_command": [
                sys.executable,
                "tools/oc133_toe_closure_factory.py",
                "--execute-comparator-source-executor",
                gap_id,
                subartifact_id,
                "--write",
            ],
            "pass_predicate": "A governed source/evidence executor exists, writes a hash-bound evidence artifact, and its downstream scoring/replay predicates pass.",
            "validator_binding": f"comparator_gap::{gap_id}::source_executor::{subartifact_id}",
            "next_escalation": "If no deterministic executor exists, implement the specific acquisition/scoring tool for this required_source_block; keep r017 blocked.",
            "no_fake_closure_policy": "This work order is not evidence and cannot close comparator broad superiority by itself.",
        },
        {
            "execution_command": [
                sys.executable,
    ...
    payload["artifact_ref"] = rel(root, root / comparator_source_executor_work_order_rel(gap_id, subartifact_id))
    payload["artifact_hash"] = artifact_hash(payload)
    write_json_artifact(root, comparator_source_executor_work_order_rel(gap_id, subartifact_id), payload)
    return payload
...
def capability_executor_for_row(row: dict[str, Any], compiled_capability_id: str) -> tuple[list[str], str, str]:
    ...
    if lane_id == "MODERN_SCIENCE_COMPARATOR_SUPERIORITY":
                ...
                if scoring_report.get("status") in {"CAPABILITY_DEVELOPMENT_REQUIRED", "EVIDENCE_REPAIR_ATTEMPTED_REMAINS_OPEN", "FAIL_CLOSED"}:
                    ...
                    if implementation_report.get("root_cause_class") == "DOMAIN_SCORER_MODEL_OR_COMPARATOR_REPAIR_REQUIRED":
                        ...
                        if repair_id in comparator_domain_model_repair_rows_by_id(ROOT) and not comparator_domain_model_repair_completed(ROOT, repair_id):
                            return (
                                [sys.executable, "tools/oc133_toe_closure_factory.py", "--execute-comparator-domain-model-repair", repair_id, "--write"],
                                "comparator_domain_model_repair",
                                "Execute the exact model/comparator repair frontier after the scorer route has already run.",
                            )
                        return (
                            [sys.executable, "tools/oc133_toe_closure_factory.py", "--compile-comparator-domain-model-repair-backlog", "--write"],
                            "comparator_domain_model_repair_backlog",
                            "Compile scorer-ran-but-open rows into component-level model/comparator repair obligations.",
                        )
                ...
                if implementation_report.get("root_cause_class") == "DOMAIN_SCORER_MODEL_OR_COMPARATOR_REPAIR_REQUIRED":
                    ...
                    if repair_id in comparator_domain_model_repair_rows_by_id(ROOT) and not comparator_domain_model_repair_completed(ROOT, repair_id):
                        return (
                            [sys.executable, "tools/oc133_toe_closure_factory.py", "--execute-comparator-domain-model-repair", repair_id, "--write"],
                            "comparator_domain_model_repair",
                            "Execute the model/comparator repair row produced by the already-run domain scorer.",
                        )
                    return (
                        [sys.executable, "tools/oc133_toe_closure_factory.py", "--compile-comparator-domain-model-repair-backlog", "--write"],
                        "comparator_domain_model_repair_backlog",
        ...
        if artifact_key in {"domain_model_repair_backlog", "domain_model_repair"}:
            if artifact_key == "domain_model_repair" and gap_id:
                repair_id = comparator_domain_model_repair_id(gap_id)
                repair_report = comparator_domain_model_repair_report(ROOT, repair_id)
                if repair_report.get("component_open_total", 0):
                    return (
                ...
                if repair_id in comparator_domain_model_repair_rows_by_id(ROOT) and not comparator_domain_model_repair_completed(ROOT, repair_id):
                    return (
                        [sys.executable, "tools/oc133_toe_closure_factory.py", "--execute-comparator-domain-model-repair", repair_id, "--write"],
                        "comparator_domain_model_repair",
                        "Execute the exact model/comparator repair row.",
                    )
            return (
                [sys.executable, "tools/oc133_toe_closure_factory.py", "--compile-comparator-domain-model-repair-backlog", "--write"],
                "comparator_domain_model_repair_backlog",
                "Compile scorer-ran-but-open rows into model/comparator repair obligations.",
            )
        if artifact_key.startswith("domain_model_component::"):
            component_id = artifact_key.split("::", 1)[1]
            component_work_order_id = comparator_domain_model_component_id(gap_id, component_id)
            if component_work_order_id in comparator_domain_model_component_rows_by_id(ROOT) and not comparator_domain_model_component_completed(ROOT, component_work_order_id):
                implementation_id = comparator_domain_model_component_implementation_id(component_work_order_id)
                ...
                if implementation_report.get("status") == "EVIDENCE_REPAIR_ATTEMPTED_REMAINS_OPEN":
                    ...
                    if obligation_id in comparator_component_source_obligation_rows_by_id(ROOT) and not comparator_component_source_obligation_completed(ROOT, obligation_id):
                        return (
                            ...
                            "comparator_component_source_obligation",
                            "Execute the narrowed source/model/comparator/residual/replay obligation after component commands ran but evidence stayed open.",
                        )
                    return (
                        [sys.executable, "tools/oc133_toe_closure_factory.py", "--compile-comparator-component-source-obligation-backlog", "--write"],
                        "comparator_component_source_obligation_backlog",
                        "Compile command-ran-but-evidence-open component reports into source-bound scoring materialization obligations.",
                    )
                if implementation_report.get("status") in {"CAPABILITY_DEVELOPMENT_REQUIRED", "EVIDENCE_REPAIR_ATTEMPTED_REMAINS_OPEN", "FAIL_CLOSED"}:
                    return (
                        ...
                        "Execute the concrete domain scorer/model implementation for this open component before rerunning generic component diagnostics.",
                    )
                if implementation_id in comparator_domain_model_component_implementation_rows_by_id(ROOT) and not comparator_domain_model_component_implementation_completed(ROOT, implementation_id):
                    return (
                        [sys.executable, "tools/oc133_toe_closure_factory.py", "--execute-comparator-domain-model-component-implementation", implementation_id, "--write"],
                        "comparator_domain_model_component_implementation",
                        "Execute the concrete domain scorer/model implementation for this open component.",
                    )
                implementation_backlog = read_json(ROOT / comparator_domain_model_component_implementation_backlog_rel())
                if implementation_backlog.get("status") != "PASS":
                    return (
                        ...
                        "Compile open component rows into concrete domain scorer/model implementation obligations.",
                    )
                return (
                    [sys.executable, "tools/oc133_toe_closure_factory.py", "--execute-comparator-domain-model-component", component_work_order_id, "--write"],
                    "comparator_domain_model_component",
                    "Execute the exact component-level model/comparator repair obligation.",
                )
            return (
                [sys.executable, "tools/oc133_toe_closure_factory.py", "--compile-comparator-domain-model-component-backlog", "--write"],
                "comparator_domain_model_component_backlog",
                "Compile open model/comparator repair components into executable obligations.",
            )
        if artifact_key.startswith("domain_model_component_implementation::"):
            component_id = artifact_key.split("::", 1)[1]
            component_work_order_id = comparator_domain_model_component_id(gap_id, component_id)
            implementation_id = comparator_domain_model_component_implementation_id(component_work_order_id)
            implementation_report = comparator_domain_model_component_implementation_report(ROOT, implementation_id)
            if implementation_report.get("status") == "EVIDENCE_REPAIR_ATTEMPTED_REMAINS_OPEN":
                ...
                if obligation_id in comparator_component_source_obligation_rows_by_id(ROOT) and not comparator_component_source_obligation_completed(ROOT, obligation_id):
                    return (
                        [sys.executable, "tools/oc133_toe_closure_factory.py", "--execute-comparator-component-source-obligation", obligation_id, "--write"],
                        "comparator_component_source_obligation",
                        "Execute the source-bound materialization obligation produced by the exhausted component implementation.",
                    )
                return (
                    [sys.executable, "tools/oc133_toe_closure_factory.py", "--compile-comparator-component-source-obligation-backlog", "--write"],
                    "comparator_component_source_obligation_backlog",
                    "Compile exhausted component implementations into exact source/model/comparator/replay obligations.",
                )
            return (
                [sys.executable, "tools/oc133_toe_closure_factory.py", "--execute-comparator-domain-model-component-implementation", implementation_id, "--write"],
                "comparator_domain_model_component_implementation",
                "Execute the concrete component implementation before source-bound materialization can be compiled.",
            )
                    ...
                    if scoring_report.get("status") in {"CAPABILITY_DEVELOPMENT_REQUIRED", "EVIDENCE_REPAIR_ATTEMPTED_REMAINS_OPEN", "FAIL_CLOSED"}:
                        ...
                        if implementation_report.get("root_cause_class") == "DOMAIN_SCORER_MODEL_OR_COMPARATOR_REPAIR_REQUIRED":
                            ...
                            return (
                                [sys.executable, "tools/oc133_toe_closure_factory.py", "--compile-comparator-domain-model-repair-backlog", "--write"],
                                "comparator_domain_model_repair_backlog",
                                "Compile scorer-ran-but-open rows into component-level model/comparator repair obligations instead of rerunning exhausted scorer attempts.",
                            )
                        if implementation_id in comparator_domain_scorer_implementation_rows_by_id(ROOT) and not comparator_domain_scorer_implementation_completed(ROOT, implementation_id):
                            return (
                                [sys.executable, "tools/oc133_toe_closure_factory.py", "--execute-comparator-domain-scorer-implementation", implementation_id, "--write"],
...
def build_capability_implementation_registry(root: Path, *, generated_at: str | None = None) -> dict[str, Any]:
    ...
    for source_row in load_autonomous_capability_development_rows(root):
        ...
        if (
            ...
            or row.get("superseded_by_domain_model_repair_backlog") is True
            or row.get("superseded_by_domain_model_repair_report") is True
            or row.get("superseded_by_domain_model_component_implementation_backlog") is True
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--compile-comparator-domain-model-repair-backlog", action="store_true", help="Compile scorer-ran-but-open reports into component-level domain model/comparator repair obligations.")
    parser.add_argument("--execute-comparator-domain-model-repair", help="Execute one component-level domain model/comparator repair obligation by id.")
    parser.add_argument("--execute-comparator-domain-model-repair-batch", action="store_true", help="Execute domain model/comparator repair obligations in deterministic order.")
    parser.add_argument("--domain-model-repair-batch-limit", type=int, default=0, help="Optional cap for domain model/comparator repair batch execution; 0 means no cap.")
    parser.add_argument("--compile-comparator-domain-model-component-backlog", action="store_true", help="Compile domain-model repair reports into exact component-level implementation obligations.")
    parser.add_argument("--execute-comparator-domain-model-component", help="Execute one component-level domain model/comparator implementation obligation by id.")
    parser.add_argument("--execute-comparator-domain-model-component-batch", action="store_true", help="Execute domain model/comparator component obligations in deterministic order.")
    parser.add_argument("--domain-model-component-batch-limit", type=int, default=0, help="Optional cap for domain model/comparator component batch execution; 0 means no cap.")
    parser.add_argument("--compile-comparator-domain-model-component-implementation-backlog", action="store_true", help="Compile component rows into concrete domain scorer/model implementation obligations.")
    parser.add_argument("--execute-comparator-domain-model-component-implementation", help="Execute one concrete domain model/comparator component implementation by id.")
    parser.add_argument("--execute-comparator-domain-model-component-implementation-batch", action="store_true", help="Execute component implementation obligations in deterministic order.")
    parser.add_argument("--domain-model-component-implementation-batch-limit", type=int, default=0, help="Optional cap for component implementation batch execution; 0 means no cap.")
    parser.add_argument("--compile-comparator-component-source-obligation-backlog", action="store_true", help="Compile command-ran-but-evidence-open component reports into exact source/model/comparator/replay obligations.")
    ...
    if args.execute_all_comparator_scoring_subartifacts:
        ...
        print(json.dumps(result if args.write or args.check else payload, ensure_ascii=False, indent=2, sort_keys=True))
        return 1 if args.check and result["state"] != "PASS" else 0
    if args.execute_comparator_source_executor:
        gap_id, subartifact_id = args.execute_comparator_source_executor
        payload = build_comparator_source_executor_work_order(ROOT, gap_id, subartifact_id)
        artifact_ref = payload.get("artifact_ref")
    ...
    if args.execute_comparator_domain_scorer_implementation_batch:
        ...
        print(json.dumps(result if args.write or args.check else payload, ensure_ascii=False, indent=2, sort_keys=True))
        return 1 if args.check and result["state"] != "PASS" else 0
    if args.compile_comparator_domain_model_repair_backlog:
        payload = build_comparator_domain_model_repair_backlog(ROOT)
        artifact_ref = payload.get("artifact_ref")
        path = ROOT / artifact_ref if isinstance(artifact_ref, str) and artifact_ref else ROOT / comparator_domain_model_repair_backlog_rel()
        result = validation_result({path: stable_json(payload)}, write=args.write)
        print(json.dumps(result if args.write or args.check else payload, ensure_ascii=False, indent=2, sort_keys=True))
        return 1 if args.check and result["state"] != "PASS" else 0
    if args.execute_comparator_domain_model_repair:
        payload = build_comparator_domain_model_repair_execution(ROOT, args.execute_comparator_domain_model_repair)
        artifact_ref = payload.get("artifact_ref")
        path = ROOT / artifact_ref if isinstance(artifact_ref, str) and artifact_ref else ROOT / comparator_domain_model_repair_rel(args.execute_comparator_domain_model_repair)
        result = validation_result({path: stable_json(payload)}, write=args.write)
        print(json.dumps(result if args.write or args.check else payload, ensure_ascii=False, indent=2, sort_keys=True))
        return 1 if args.check and result["state"] != "PASS" else 0
    if args.execute_comparator_domain_model_repair_batch:
        payload = execute_comparator_domain_model_repair_batch(ROOT, limit=args.domain_model_repair_batch_limit)
        path = ROOT / lane_execution_base("MODERN_SCIENCE_COMPARATOR_SUPERIORITY") / "DOMAIN_MODEL_REPAIR_BATCH.json"
        result = validation_result({path: stable_json(payload)}, write=args.write)
        print(json.dumps(result if args.write or args.check else payload, ensure_ascii=False, indent=2, sort_keys=True))
        return 1 if args.check and result["state"] != "PASS" else 0
    if args.compile_comparator_domain_model_component_backlog:
        payload = build_comparator_domain_model_component_backlog(ROOT)
        artifact_ref = payload.get("artifact_ref")
        path = ROOT / artifact_ref if isinstance(artifact_ref, str) and artifact_ref else ROOT / comparator_domain_model_component_backlog_rel()
        result = validation_result({path: stable_json(payload)}, write=args.write)
        print(json.dumps(result if args.write or args.check else payload, ensure_ascii=False, indent=2, sort_keys=True))
        return 1 if args.check and result["state"] != "PASS" else 0
    if args.execute_comparator_domain_model_component:
        payload = build_comparator_domain_model_component_execution(ROOT, args.execute_comparator_domain_model_component)
        artifact_ref = payload.get("artifact_ref")
        path = ROOT / artifact_ref if isinstance(artifact_ref, str) and artifact_ref else ROOT / comparator_domain_model_component_rel(args.execute_comparator_domain_model_component)
        result = validation_result({path: stable_json(payload)}, write=args.write)
        print(json.dumps(result if args.write or args.check else payload, ensure_ascii=False, indent=2, sort_keys=True))
        return 1 if args.check and result["state"] != "PASS" else 0
    if args.execute_comparator_domain_model_component_batch:
        payload = execute_comparator_domain_model_component_batch(ROOT, limit=args.domain_model_component_batch_limit)
        path = ROOT / lane_execution_base("MODERN_SCIENCE_COMPARATOR_SUPERIORITY") / "DOMAIN_MODEL_COMPONENT_BATCH.json"
        result = validation_result({path: stable_json(payload)}, write=args.write)
        print(json.dumps(result if args.write or args.check else payload, ensure_ascii=False, indent=2, sort_keys=True))
        return 1 if args.check and result["state"] != "PASS" else 0
    if args.compile_comparator_domain_model_component_implementation_backlog:
        payload = build_comparator_domain_model_component_implementation_backlog(ROOT)
        build_comparator_domain_model_component_implementation_registry(ROOT)
        artifact_ref = payload.get("artifact_ref")
        path = ROOT / artifact_ref if isinstance(artifact_ref, str) and artifact_ref else ROOT / comparator_domain_model_component_implementation_backlog_rel()
        result = validation_result({path: stable_json(payload)}, write=args.write)
        print(json.dumps(result if args.write or args.check else payload, ensure_ascii=False, indent=2, sort_keys=True))
        return 1 if args.check and result["state"] != "PASS" else 0
    if args.execute_comparator_domain_model_component_implementation:
        payload = build_comparator_domain_model_component_implementation_execution(ROOT, args.execute_comparator_domain_model_component_implementation, timeout=args.timeout)
        build_comparator_domain_model_component_implementation_registry(ROOT)
        artifact_ref = payload.get("artifact_ref")
        path = ROOT / artifact_ref if isinstance(artifact_ref, str) and artifact_ref else ROOT / comparator_domain_model_component_implementation_rel(args.execute_comparator_domain_model_component_implementation)
        result = validation_result({path: stable_json(payload)}, write=args.write)
        print(json.dumps(result if args.write or args.check else payload, ensure_ascii=False, indent=2, sort_keys=True))
        return 1 if args.check and result["state"] != "PASS" else 0
    if args.execute_comparator_domain_model_component_implementation_batch:
        payload = execute_comparator_domain_model_component_implementation_batch(ROOT, timeout=args.timeout, limit=args.domain_model_component_implementation_batch_limit)
        path = ROOT / lane_execution_base("MODERN_SCIENCE_COMPARATOR_SUPERIORITY") / "DOMAIN_MODEL_COMPONENT_IMPLEMENTATION_BATCH.json"
        result = validation_result({path: stable_json(payload)}, write=args.write)
        print(json.dumps(result if args.write or args.check else payload, ensure_ascii=False, indent=2, sort_keys=True))
        return 1 if args.check and result["state"] != "PASS" else 0
    if args.compile_comparator_component_source_obligation_backlog:
        payload = build_comparator_component_source_obligation_backlog(ROOT)
        build_comparator_component_source_obligation_registry(ROOT)
        artifact_ref = payload.get("artifact_ref")
        path = ROOT / artifact_ref if isinstance(artifact_ref, str) and artifact_ref else ROOT / comparator_component_source_obligation_backlog_rel()
        result = validation_result({path: stable_json(payload)}, write=args.write)
        print(json.dumps(result if args.write or args.check else payload, ensure_ascii=False, indent=2, sort_keys=True))
        return 1 if args.check and result["state"] != "PASS" else 0
    if args.execute_comparator_component_source_obligation:
        payload = build_comparator_component_source_obligation_execution(ROOT, args.execute_comparator_component_source_obligation, timeout=args.timeout)
        build_comparator_component_source_obligation_registry(ROOT)
        artifact_ref = payload.get("artifact_ref")
        path = ROOT / artifact_ref if isinstance(artifact_ref, str) and artifact_ref else ROOT / comparator_component_source_obligation_rel(args.execute_comparator_component_source_obligation)
        result = validation_result({path: stable_json(payload)}, write=args.write)
        print(json.dumps(result if args.write or args.check else payload, ensure_ascii=False, indent=2, sort_keys=True))
        return 1 if args.check and result["state"] != "PASS" else 0
