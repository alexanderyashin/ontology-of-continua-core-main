...
def main() -> int:
    ...
    if args.materialize_first:
        materialize = subprocess.run(
            [sys.executable, str(ROOT / "validation" / "run_all.py")],
            cwd=ROOT,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    if target_script.exists():
        ...
        if target_path.exists():
            ...
            if target_payload.get("generated_by") != "PUBLIC_VALIDATION_WORKER":
                target_blind_failures.append("TARGET_BLIND_NOT_PUBLIC_CAPABILITY_GENERATED")
            if target_payload.get("evidence_owner") != "Research/EmpiricalScience":
                target_blind_failures.append("TARGET_BLIND_EVIDENCE_OWNER_MISMATCH")
    ...
    payload = {
        ...
        "target_blind_evidence_owner": target_payload.get("evidence_owner"),
