...
SOURCE_SCHEMA_ID = "OC133_SYSTEMS_WDI_GDP_SOURCE_SNAPSHOT_v1"
LOCK_SCHEMA_ID = "OC133_SYSTEMS_WDI_GDP_SOURCE_LOCK_v1"
TASK_SCHEMA_ID = "OC133_SYSTEMS_WDI_GDP_TARGET_HIDDEN_TASK_TABLE_v1"
TARGET_LOCK_SCHEMA_ID = "OC133_SYSTEMS_WDI_GDP_HIDDEN_TARGET_LOCK_v1"
PACK_SCHEMA_ID = "OC133_SYSTEMS_WDI_GDP_SCORING_PACK_v1"
REPLAY_SCHEMA_ID = "OC133_SYSTEMS_WDI_GDP_REPLAY_REPORT_v1"

SCRIPT_REL = (
    "validation/heldout/grand_science/systems/coverage_work_orders/"
    "oc133_systems_wdi_gdp_materializer.py"
)
ROOT_REL = "validation/heldout/grand_science/systems/wdi_gdp"
SNAPSHOT_REL = f"{ROOT_REL}/OC133_WORLD_BANK_WDI_GDP_CONSTANT_SOURCE.json"
LOCK_REL = f"{ROOT_REL}/OC133_WORLD_BANK_WDI_GDP_CONSTANT_SOURCE.lock.json"
TASK_TABLE_REL = f"{ROOT_REL}/OC133_WORLD_BANK_WDI_GDP_TARGET_HIDDEN_TASK_TABLE.json"
TARGET_LOCK_REL = f"{ROOT_REL}/OC133-WORLD-BANK-WDI-GDP-HIDDEN-TARGETS-0001.lock.json"
SCORING_PACK_REL = f"{ROOT_REL}/OC133_WORLD_BANK_WDI_GDP_TARGET_HIDDEN_REPLAY_SCORER_EVIDENCE_PACK.json"
REPLAY_REPORT_REL = f"{ROOT_REL}/OC133_WORLD_BANK_WDI_GDP_REPLAY_REPORT.json"

SOURCE_ID = "world_bank_wdi_gdp_constant_2015_usd_v1"
MODEL_ID = "SYS-WDI-GDP-LOG-LINEAR-FLOW-CONTINUATION-v1"
COMPARATOR_ID = "SYS-WDI-GDP-LAST-OBSERVATION-BASELINE-v1"
NEGATIVE_CONTROL_ID = "SYS-WDI-GDP-FIVE_YEAR_STALE_BASELINE_NEGATIVE_CONTROL-v1"
INDICATOR = "NY.GDP.MKTP.KD"
INDICATOR_LABEL = "GDP (constant 2015 US$)"
API_URL = (
...
HISTORY_YEARS = 5
MINIMUM_ROWS = 200
MATERIAL_MARGIN_RELATIVE_MAE = 0.005
...
def acquire(root: Path, *, write: bool) -> dict[str, Any]:
    ...
    for item in raw[1]:
        ...
        try:
            ...
            gdp_constant_2015_usd = float(value)
        except (TypeError, ValueError):
            continue
        if gdp_constant_2015_usd <= 0:
            continue
        row = {
            ...
            "year": year,
            "gdp_constant_2015_usd": gdp_constant_2015_usd,
            "row_sha256": "",
        }
    ...
    lock = {
        ...
        "target_fields_hidden": ["gdp_constant_2015_usd"],
        "target_values_scored": False,
        "target_hidden_until_scoring": True,
        "coverage_closure_allowed": False,
    }
...
def rows_by_country(snapshot: dict[str, Any]) -> dict[str, dict[int, float]]:
    ...
    for row in snapshot.get("rows", []):
        ...
        try:
            ...
            value = float(row.get("gdp_constant_2015_usd"))
        except (TypeError, ValueError):
            continue
        ...
        for target_year in HOLDOUT_YEARS:
            ...
            for year in range(target_year - HISTORY_YEARS, target_year):
                if yearly.get(year) is not None:
                    history.append({"year": year, "gdp_constant_2015_usd": yearly[year]})
            target = yearly.get(target_year)
            if len(history) != HISTORY_YEARS or target is None:
                continue
            task_id = f"SYS-WDI-GDP-HOLDOUT-{country}-{target_year}"
            visible_rows.append(
                {
                    "task_id": task_id,
            ...
            hidden_rows.append(
                {
                    "task_id": task_id,
                    "target_gdp_constant_2015_usd": target,
                    "source_row_sha256": source_hash_by_key.get((country, target_year)),
                }
            )
    task_table = {
        ...
        "model": {
            "model_id": MODEL_ID,
            "rule": "Fit a five-year log-linear country/region GDP flow to pre-target years and extrapolate one year ahead.",
        },
        "comparator": {
            "comparator_id": COMPARATOR_ID,
            "rule": "Predict held-out GDP as the last visible pre-target GDP value.",
        },
...
def log_linear_predict(history_rows: list[dict[str, Any]], target_year: int) -> float:
    ...
    ys = [math.log(float(row["gdp_constant_2015_usd"])) for row in history_rows]
...
def score(root: Path, *, write: bool) -> dict[str, Any]:
    ...
    for row in task_table["visible_rows"]:
        ...
        target = float(hidden_by_id[row["task_id"]]["target_gdp_constant_2015_usd"])
        model_prediction = log_linear_predict(history, int(row["target_year"]))
        comparator_prediction = float(history[-1]["gdp_constant_2015_usd"])
        negative_control_prediction = float(history[0]["gdp_constant_2015_usd"])
        ...
        scored.append(
            {
                ...
                "model_prediction_gdp_constant_2015_usd": round(model_prediction, 2),
                "comparator_prediction_gdp_constant_2015_usd": round(comparator_prediction, 2),
                "target_gdp_constant_2015_usd_opened_after_scoring": round(target, 2),
    ...
    pack = {
        ...
        "residuals": {
            ...
            "material_margin_rule": "model_relative_mae + 0.005 must be below last-observation comparator relative MAE",
        },
        "negative_control": {
            "control_id": NEGATIVE_CONTROL_ID,
            "negative_control_rejected": negative_control_rejected,
            "rule": "Five-year stale GDP baseline must be materially worse than the preregistered model and comparator.",
        },
        "falsifiers": [
            "World Bank WDI snapshot hash cannot be reproduced",
            "held-out GDP target leaks into the five-year model fit",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Materialize World Bank WDI GDP target-hidden replay evidence.")
    parser.add_argument("--root", default=str(repo_root()), help="repository root")
    parser.add_argument("--acquire", action="store_true", help="fetch and lock the WDI GDP source snapshot")
    parser.add_argument("--write-acquisition", action="store_true", help="write acquisition artifacts")
    parser.add_argument("--score", action="store_true", help="score the target-hidden WDI GDP replay")
