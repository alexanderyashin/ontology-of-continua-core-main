...
SOURCE_SCHEMA_ID = "OC133_MEDICAL_WDI_CHILD_MORTALITY_SOURCE_SNAPSHOT_v1"
LOCK_SCHEMA_ID = "OC133_MEDICAL_WDI_CHILD_MORTALITY_SOURCE_LOCK_v1"
TASK_SCHEMA_ID = "OC133_MEDICAL_WDI_CHILD_MORTALITY_TARGET_HIDDEN_TASK_TABLE_v1"
TARGET_LOCK_SCHEMA_ID = "OC133_MEDICAL_WDI_CHILD_MORTALITY_HIDDEN_TARGET_LOCK_v1"
PACK_SCHEMA_ID = "OC133_MEDICAL_WDI_CHILD_MORTALITY_SCORING_PACK_v1"
REPLAY_SCHEMA_ID = "OC133_MEDICAL_WDI_CHILD_MORTALITY_REPLAY_REPORT_v1"

SCRIPT_REL = (
    "validation/heldout/grand_science/medical/coverage_work_orders/"
    "oc133_medical_wdi_child_mortality_materializer.py"
)
ROOT_REL = "validation/heldout/grand_science/medical/wdi_child_mortality"
SNAPSHOT_REL = f"{ROOT_REL}/OC133_WORLD_BANK_WDI_CHILD_MORTALITY_SOURCE.json"
LOCK_REL = f"{ROOT_REL}/OC133_WORLD_BANK_WDI_CHILD_MORTALITY_SOURCE.lock.json"
TASK_TABLE_REL = f"{ROOT_REL}/OC133_WORLD_BANK_WDI_CHILD_MORTALITY_TARGET_HIDDEN_TASK_TABLE.json"
TARGET_LOCK_REL = f"{ROOT_REL}/OC133-WORLD-BANK-WDI-CHILD-MORTALITY-HIDDEN-TARGETS-0001.lock.json"
SCORING_PACK_REL = f"{ROOT_REL}/OC133_WORLD_BANK_WDI_CHILD_MORTALITY_TARGET_HIDDEN_REPLAY_SCORER_EVIDENCE_PACK.json"
REPLAY_REPORT_REL = f"{ROOT_REL}/OC133_WORLD_BANK_WDI_CHILD_MORTALITY_REPLAY_REPORT.json"

SOURCE_ID = "world_bank_wdi_under_five_mortality_v1"
MODEL_ID = "MED-WDI-CHILD-MORTALITY-LOG_LINEAR_RISK_DECLINE-v1"
COMPARATOR_ID = "MED-WDI-CHILD-MORTALITY-LAST_OBSERVATION_BASELINE-v1"
NEGATIVE_CONTROL_ID = "MED-WDI-CHILD-MORTALITY-FIVE_YEAR_MEAN_NEGATIVE_CONTROL-v1"
...
HISTORY_YEARS = 5
MINIMUM_ROWS = 500
MATERIAL_MARGIN_RELATIVE_MAE = 0.01
...
def acquire(root: Path, *, write: bool) -> dict[str, Any]:
    ...
    for item in raw[1]:
        ...
        try:
            ...
            mortality_per_1000 = float(value)
        except (TypeError, ValueError):
            continue
        if mortality_per_1000 <= 0:
            continue
        row = {
            ...
            "year": year,
            "under_five_mortality_per_1000": mortality_per_1000,
            "row_sha256": "",
        }
    ...
    lock = {
        ...
        "target_fields_hidden": ["under_five_mortality_per_1000"],
        "target_values_scored": False,
        "target_hidden_until_scoring": True,
        "coverage_closure_allowed": False,
    }
...
def rows_by_country(snapshot: dict[str, Any]) -> dict[str, dict[int, float]]:
    ...
    for row in snapshot.get("rows", []):
        ...
        try:
            ...
            value = float(row.get("under_five_mortality_per_1000"))
        except (TypeError, ValueError):
            continue
...
def build_task_table(snapshot: dict[str, Any], lock: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    for country, yearly in sorted(by_country.items()):
        for target_year in HOLDOUT_YEARS:
            history = [
                {"year": year, "under_five_mortality_per_1000": yearly[year]}
            ...
            if len(history) != HISTORY_YEARS or target is None:
                continue
            task_id = f"MED-WDI-U5MR-HOLDOUT-{country}-{target_year}"
            visible_rows.append(
                {
                    "task_id": task_id,
            ...
            hidden_rows.append(
                {
                    "task_id": task_id,
                    "target_under_five_mortality_per_1000": target,
                    "source_row_sha256": source_hash_by_key.get((country, target_year)),
                }
            )
    task_table = {
        ...
        "model": {
            "model_id": MODEL_ID,
            "rule": "Fit a five-year log-linear mortality-risk decline and extrapolate one year.",
        },
        "comparator": {
            "comparator_id": COMPARATOR_ID,
            "rule": "Predict held-out mortality risk as the last visible pre-target value.",
        },
...
def log_linear_predict(history_rows: list[dict[str, Any]], target_year: int) -> float:
    ...
    ys = [math.log(float(row["under_five_mortality_per_1000"])) for row in history_rows]
...
def score(root: Path, *, write: bool) -> dict[str, Any]:
    ...
    for row in task_table["visible_rows"]:
        ...
        target = float(hidden_by_id[row["task_id"]]["target_under_five_mortality_per_1000"])
        model_prediction = log_linear_predict(history, int(row["target_year"]))
        comparator_prediction = float(history[-1]["under_five_mortality_per_1000"])
        negative_control_prediction = math.exp(
            sum(math.log(float(item["under_five_mortality_per_1000"])) for item in history) / len(history)
        )
        ...
        scored.append(
            {
                ...
                "model_prediction_per_1000": round(model_prediction, 8),
                "comparator_prediction_per_1000": round(comparator_prediction, 8),
                "target_per_1000_opened_after_scoring": round(target, 8),
    ...
    pack = {
        ...
        "residuals": {
            ...
            "material_margin_rule": "model_relative_mae + 0.01 must be below last-observation comparator relative MAE",
        },
        "negative_control": {
            "control_id": NEGATIVE_CONTROL_ID,
            "negative_control_rejected": negative_control_rejected,
            "rule": "Five-year geometric mean mortality risk must be worse than the trend model and comparator.",
        },
        "falsifiers": [
            "World Bank WDI source hash cannot be reproduced",
            "held-out child mortality target leaks into the five-year model fit",
            "last-observation comparator ties or beats the log-linear risk-decline model within the material margin",
            "five-year mean negative control is not rejected",
        ],
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Materialize World Bank WDI under-five mortality target-hidden replay evidence.")
    parser.add_argument("--root", default=str(repo_root()), help="repository root")
    parser.add_argument("--acquire", action="store_true", help="fetch and lock the WDI child mortality source snapshot")
    parser.add_argument("--write-acquisition", action="store_true", help="write acquisition artifacts")
    parser.add_argument("--score", action="store_true", help="score the target-hidden child mortality replay")
