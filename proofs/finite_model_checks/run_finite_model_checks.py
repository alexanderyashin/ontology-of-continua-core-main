...
PUBLIC_APPROVAL_MUTABLE_REFS = {
    "release_policy/OC_CORE_1_4_PUBLIC_NO_SEND_MANIFEST.json",
    "release_policy/PUBLIC_RELEASE_APPROVAL_PENDING.json",
}
...
def release_critical_source_refs() -> list[str]:
    return [
        ...
        "benchmarks/modern_science/validate_modern_science_register.py",
        "benchmarks/modern_science/clean_checkout_replay.py",
...
def active_public_release_approval_mode() -> bool:
    try:
        manifest = read_rel_json("release_policy/OC_CORE_1_4_PUBLIC_NO_SEND_MANIFEST.json")
    except Exception:
        manifest = {}
    return (
        manifest.get("approved") is True
...
def public_release_approval_mode() -> bool:
    ...
    try:
        grant_payload = read_rel_json("release_policy/PUBLIC_RELEASE_APPROVAL_GRANTED.json")
    except Exception:
        grant_payload = {}
    grant = grant_payload.get("approval", {}) if isinstance(grant_payload, dict) else {}
    durable_grant = (
        grant.get("approved") is True
...
def hypothetical_approved_control(model: dict[str, Any]) -> str:
    approved = model.get("approved") is True
    ...
    if "fresh_public_review_required_for_release" in model:
        fresh_review_open = (
            model.get("fresh_public_review_required_for_release") is True
...
def observed(row: dict[str, Any]) -> str:
    ...
    if case_type == "no_send_state_machine":
        ...
        approval_locked = (
            ...
            and approval.get("approval_required") is True
    ...
    if case_type == "no_send_hypothetical_control":
        return hypothetical_approved_control(model)
    return "UNKNOWN"
...
def evaluate(row: dict[str, Any]) -> dict[str, Any]:
    ...
    if row.get("theorem_id") == "OC133-NOSEND-001":
        ...
        if model.get("approval_ref"):
            out["public_release_approval_ref"] = model.get("approval_ref")
            out["public_release_approval_sha256"] = sha256_file(ROOT / str(model.get("approval_ref")))
            out["expected_public_release_approval_sha256"] = model.get("approval_sha256")
...
def main() -> int:
    ...
    for row in rows:
        if row.get("case_type") == "no_send_state_machine":
            ...
            approval_ref = row.get("public_release_approval_ref")
            ...
            row["public_release_approval_clean_source_byte_sha256"] = cert_manifest_byte_hashes.get(approval_ref)
            ...
            if row.get("public_release_approval_sha256") != cert_manifest_byte_hashes.get(approval_ref):
