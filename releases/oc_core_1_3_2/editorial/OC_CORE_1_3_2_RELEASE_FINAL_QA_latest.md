...
- `PASS` `LRGEF_ALIGNED_WITH_SCIENCE_TERMINALITY` observed=`{'release_machine_state': 'RELEASE_READY_NO_SEND', 'master_verdict': 'PASS', 'expected': 'RELEASE_READY_NO_SEND'}`
...
- `PASS` `SCIENCE_TERMINALITY_82_PUBLICATION_GATE` observed=`{'open_blocker_total': 0, 'science_terminality_status': 'PASS'}`
