% OC Core 1.3 -- module architecture and cross-level structures
...
bridge between formal doctrine and the Core~1.3 closure chapters.

The extended modules are:

\begin{itemize}
\item \textbf{K-levels master} (Section~\ref{sec:klevels-master}):
  global overview of all continua \(K_0\)–\(K_{12}\), their state spaces,
  axes, thresholds and core processes.

\item \textbf{Embedding spaces / M-spaces} (Section~\ref{sec:mspaces-overview}):
  definition of meta-spaces \(M_x\) that constrain and parameterise the
  evolution of continua \(K_x\).

\item \textbf{Cross-level / cross-K structures} (Section~\ref{sec:crossk_master}):
...
\item \textbf{Processes} (Section~\ref{sec:processes-master}):
