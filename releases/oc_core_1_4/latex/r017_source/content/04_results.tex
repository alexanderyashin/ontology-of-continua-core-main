...
This section summarises the core structural results that follow from the formal
framework introduced in Section~\ref{sec:model}. It presents the canonical
consequences of the Ontology of Continua (OC) axioms and operators in their
Core~1.3 form.
...
Proof support is routed through the theorem roadmap, the proof-machinery
appendix, source-audit rows, and release evidence ledgers. This chapter states
the load-bearing result forms and points to those proof surfaces; correctness is
not deferred to future extension papers.
The focus is on stating the key theorems and consequences that constrain all
continua in the hierarchy \(K_0\)–\(K_{12}\).
...
\label{sec:theorem-spontaneous-dimension-creation}
...
By definition of \(\Theta_{\mathrm{dim}}\), below this threshold all
differences can be projected onto the structural span
\(\mathrm{span}_{\mathrm{str}}(A(K_x))\); above it, projection fails and a new
axis is required.
...
\begin{enumerate}
    ...
    \item continuumness collapses, \(k(K,t) \to 0\);
...
\[
    ...
    \qquad
    \mathrm{span}_{\mathrm{str}}\!\big(A(K_x(t+dt))\big) \subseteq
    \mathrm{span}_{\mathrm{str}}\!\big(A(M_x)\big).
\]
