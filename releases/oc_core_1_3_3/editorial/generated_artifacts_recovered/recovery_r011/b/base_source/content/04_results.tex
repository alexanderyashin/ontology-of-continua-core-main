...
Promotion rule. The route may appear as a public theorem only when the promoted statement names its assumptions, proof route, executable witness or replay row when relevant, comparator boundary, and falsifier. Without those items it remains a background route in the monograph and cannot be used as evidence for final full-scope synthesis, full-domain prediction, or unrestricted comparative claim over all modern science.
...
The current promoted theorem surface is the T133 proof suite and its supporting artifacts: typed foundation, proof sheets, Lean subset, finite semantic checks, and claim-boundary records. The historical routes in this chapter help explain the origin of the questions, but the T133 artifacts carry the current release claims.
