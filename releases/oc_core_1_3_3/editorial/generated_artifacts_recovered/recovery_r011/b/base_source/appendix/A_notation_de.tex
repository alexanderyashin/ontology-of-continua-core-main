...
throughout the Core~1.3.3 monograph.  It is not exhaustive, but it covers the
...
\begin{description}
  ...
  \item[\(H(\Omega(K_x))\)] Legacy K-level notation for the Boolean
        nonemptiness factor of the admissible region; it is not the threshold
        operator \(H_{\mathrm{thr}}\).
  \item[\(\tau_{\mathrm{eff}}(K_x)\)] Scalar effective time of a level. When a
        K-level chapter locally writes scalar \(\tau(K_x)\), it is read as an
        alias for \(\tau_{\mathrm{eff}}(K_x)\), not as the full family of
        temporal parameters.
\end{description}
...
\begin{description}
  ...
  \item[\(G\)] Potential operator:
        \(G: P(t)\mapsto P(t+dt)\).
  \item[\(H_{\mathrm{thr}}\)] Threshold operator, often written locally as
        \(H\) after the chapter has bound the symbol:
  ...
  \item[\(\Psi_{x\to y}\)] Non-adjacent birth-route notation for \(y>x\). By
        default it is not a new primitive, but an abbreviation for the admissible
        composition \(\Psi_{y-1\to y}\circ\cdots\circ\Psi_{x\to x+1}\), with
        every intermediate threshold and embedding obligation preserved.
...
only in particular extension Artikels are defined locally in those texts.
