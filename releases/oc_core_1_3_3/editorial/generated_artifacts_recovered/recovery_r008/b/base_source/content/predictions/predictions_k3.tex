...
Predictions for $K_3$ rest on the expanded state space $\Omega(K_3)$
and on the geometry of two-dimensional neighbourhoods. They also use
the threshold structure $\Theta_3$ and the evolution operator $U$
acting on fields $\phi(x,y,\tau)$.
...
The scoped evolution operator must contain:
\[
