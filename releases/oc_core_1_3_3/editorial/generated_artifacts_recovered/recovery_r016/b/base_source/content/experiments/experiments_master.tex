...
The experiment chapters in this release specify proposed protocols and review designs. They do not report completed empirical support unless a row names a public source snapshot, formula or reconstruction rule, comparator, residual or uncertainty, negative control, falsifier, and replay hash.
...
across the declared K-level sequence $K_0$–$K_{12}$.
Unlike domain–specific experiments contained in the per–level files
(the K-level experiment design series), the present master module outlines the
general methodology, protocol review strategies, and cross–domain principles
...
Experimental design spans physical, chemical, biological, cognitive,
institutional, and computational modalities. Across those domains, every
experiment still receives one structural interpretation in terms of
...
\begin{enumerate}
    ...
    \item \textbf{Cycles maintain persistence.}
          Stable cycles $C$ underpin viability; their destruction may indicate or
          marks collapse.
...
\subsection{scoped Experiment Structure}
...
OC proposes scoped measurement protocols for declared continuum instances:
...
\begin{enumerate}
    ...
    \item \textbf{Cross–Domain Comparisons.}
          probe invariants across continuum levels.
\end{enumerate}
...
\subsection{Experimental protocol review Pipeline}

The OC protocol review pipeline contains four scoped stages:
...
The master falsifiability module (the falsifiability chapter)
...
This scoped generality is a defining feature of OC and a central criterion for
falsifiable theory construction:
a structural prediction supported in the scoped physics replay lane (K₂) may be tested for a corresponding scoped instantiation in chemistry (K₃–K₄), biology (K₄–K₅), cognition (K₆), social
...
The following files (organized by the canonical experiment taxonomy)
contain domain–specific experimental designs:

\begin{itemize}
    \item K0 experiment design — substrate–level tests;
    \item K1 experiment design — one–dimensional continua;
    \item K2 experiment design — physical continua;
    \item K3 experiment design — chemical RAF/network experiments;
    \item K4 experiment design — membrane thresholds \& cycles;
    \item K5 experiment design — early excitability \& proto–spikes;
    \item K6 experiment design — cognitive thresholds;
    \item K7 experiment design — institutional tests;
    \item K8 experiment design — civilizational dynamics;
    \item K9 experiment design — theoretical stability;
...
These specialised modules implement the scoped methodology presented here.
