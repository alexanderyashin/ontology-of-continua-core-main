...
\[
    K_1 = \big(X_1,\tau_1,\Omega_1,A_1,P_1(\lambda),J_1(\lambda),\Theta_1,\partial\Omega_1,C_1,k_1(\lambda)\big),
\]
where:
\begin{itemize}
    ...
    \item \(\Omega_1\) is a space of admissible configurations on \(X_1\) with appropriate regularity conditions.
          In this ordering-dependent analytic example,
          \[
              \Omega_1 = C^0(I,H^1(X_1,V_1)) \cap C^1(I,L^2(X_1,V_1)),
          \]
          where \(I\) is the local ordering interval and \(V_1\) is the Hilbert space
          ...
          freedom.
    \item \(P_1(\lambda)\) is an energy-like potential functional on \(\Omega_1\);
    \item \(J_1(\lambda)\) are flows derived from the variation of \(P_1(\lambda)\);
    ...
    \item \(k_1(\lambda)\) measures one-dimensional continuumness according to the general definition below.
\end{itemize}
...
\[
    K = \big(\Omega(K), \partial\Omega(K), A(K), \Theta(K), P(t), J(t), C(K), k(K,t), M(K)\big),
\]
where:
\begin{itemize}
    ...
    \item \(M(K)\) is the embedding context or meta-space that constrains the local continuum.
\end{itemize}
...
The canonical public tuple is \(K=(\Omega,\partial\Omega,A,\Theta,P,J,C,k,M)\). Older local displays may use a shorter or differently ordered tuple when a component is derived in that local model. Those displays are projections of the canonical release tuple, not competing definitions.
...
\]
This displayed boundary equation is therefore not primitive for the declared release domains. It
...
or, in expanded form,
\[
    E:\ (\Omega,\partial\Omega,A,\Theta,P,J,C,k,M)
    \longrightarrow (\Omega',\partial\Omega',A',\Theta',P',J',C',k',M').
\]
...
The tuple \(K=(\Omega,\partial\Omega,A,\Theta,P,J,C,k,M)\) is the public interface
used by this release. Core 1.3.3 no longer promotes a public theorem of global
verdict-invariant minimality. Instead it records a route: each tuple component
has a witness-replay row in \texttt{proofs/minimality/WITNESS\_PAIRS.json}, and the
claim register marks \(T133\)-MIN as
\(\mathrm{DEMOTED\_TO\_ROUTE\_NO\_PUBLIC\_PROMOTION}\).
...
machine-readable witness fixtures and an executable replay check for that
route, but it does not claim a complete formal lower-bound theorem.

\paragraph{Use in this release.}
...
information. Derivation is allowed; information loss is not. This is a route
constraint for reviewer orientation, not a promoted minimality proof.
