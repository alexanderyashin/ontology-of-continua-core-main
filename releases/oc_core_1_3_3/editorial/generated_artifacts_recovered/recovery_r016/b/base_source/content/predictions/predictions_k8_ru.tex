...
\subsubsection{Bounded Prediction Review for \texorpdfstring{$K_8$}{K_8}}
\label{sec:predictions-k8-r014}

At \(K_8\), prediction language is treated as a source-grounded review
target for civilizational and infrastructure systems, not as an unrestricted theorem about every system in that
...
\paragraph{Claim.}
A \(K_8\) prediction may be considered only under declared assumptions
...
A safe \(K_8\) row has the form
\[
  \mathcal{P}_{8}:
  (K_8, A_x, \Theta_x, P_x, J_x, C_x, k_x, M_x)
  \longrightarrow
...
For \(K_8\), the current release records prediction rows as bounded
