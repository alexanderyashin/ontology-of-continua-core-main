...
the formal model chapter into the full operator family used in the Ontology of
Continua (OC). It consolidates the definitions developed in archival source corpus and
presents them in a compact, vertically consistent form suitable for Core~1.3.3.
...
\[
  ...
  \big),
\]
with the components defined in the formal model chapter. The embedding space is
...
Cycles \(C(t)\) represent declared maintenance, recurrence, renewal,
proof-replay, regulatory, or geometric trajectory modes that maintain the
organisation of the continuum. A closed trajectory is the smooth/geometric
special case, not the primitive definition. Their evolution is governed by
\[
  C(t+dt) = Q\big(C(t), P(t), J(t), \Theta(t)\big).
\]
...
\begin{itemize}
  \item \emph{birth} of new cycles when the declared update mode closes,
        replays, maintains, or recurs away from the relevant boundary;
...
Theorem~9 in the historical result-boundary chapter states that death coincides with
...
The boundary operator \(R\) controls the typed admissibility classifiers that
define \(\partial\Omega\). Geometry and position are quantitative
specializations of this classifier update:
\[
  \partial\Omega(t+dt) =
...
the boundary geometry chapter provides an extended treatment of boundary
...
\(A_{\mathrm{new}} \notin \mathrm{span}_{\mathrm{str}}(A(t))\), then a dimensional transition
...
the birth operators below discussion.
...
Using the unified definition of \(k(t)\) from the formal model chapter, \(U\)
...
\begin{enumerate}
  \item \textbf{New differences.}\
        There exists a class of differences outside the current structural
        span:
        \[
          D_{\mathrm{new}}\not\subseteq
...
death of continua across all levels \(K_0\)–\(K_{12}\).

This chapter completes the formal core of Core~1.3.3 by making explicit the
