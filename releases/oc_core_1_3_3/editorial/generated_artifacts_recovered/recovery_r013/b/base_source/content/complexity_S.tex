...
% ============================
\section{Complexity Metric \texorpdfstring{\(S(K)\)}{S(K)}}
...
\end{itemize}
Functionally, the scoped operator \(U\) governing \(dk/dt\) depends on \(S(K)\):
\[
...
Taken together, the components \(S_\Omega,S_A,S_C,S_\Theta,S_J\) provide a quantitative measure of complexity \(S(K)\) that supports the formal description of growth, phase transitions, and death of continua within the bounded Core 1.3.3 release scope.
