...
\label{sec:r006-continuum-problem}
...
\begin{tikzpicture}[scale=0.92, every node/.style={font=\small}]
  ...
  \draw[rounded corners=12pt, line width=1pt, ocBlue] (-4.2,-2.1) rectangle (4.2,2.1);
  \node[ocBlue, font=\bfseries] at (0,2.45) {Continuum \(K\) inside an embedding context \(M\)};
  \draw[->, line width=0.8pt, ocGold] (-3.5,-1.4) -- (3.4,-1.4) node[right] {axis \(A_1\)};
  \draw[->, line width=0.8pt, ocGold] (-3.5,-1.4) -- (-3.5,1.55) node[above] {axis \(A_2\)};
  \draw[fill=ocGreen!14, draw=ocGreen, line width=0.8pt] (0,0) ellipse (2.55 and 1.18);
  \node at (0,0.05) {admissible states \(\Omega(K)\)};
  \draw[dashed, line width=0.7pt, red!70!black] (2.55,0) arc (0:360:2.55 and 1.18);
  \node[red!70!black] at (2.95,1.1) {boundary \(\partial\Omega\)};
  \draw[->, thick, blue!70!black] (-2.1,-0.4) .. controls (-1.2,1.2) and (0.8,1.2) .. (1.8,-0.25);
  \node[blue!70!black] at (0,1.45) {flow \(J(t)\)};
  \draw[->, thick, ocGold] (1.7,-1.55) -- (1.7,-0.65);
  \node[ocGold] at (2.65,-1.25) {threshold \(\Theta\)};
  \draw[->, line width=0.8pt] (-0.8,-0.5) arc (215:-80:0.72);
  \node at (-0.2,-0.92) {cycle \(C\)};
  \node[align=center] at (-2.75,1.2) {\(k(K,t)>0\)\\ live continuum};
\end{tikzpicture}
...
continuumness condition. It is not a domain-specific picture; it is the common
structural template used later for physical, biological, cognitive, social,
enterprise, and theoretical systems.}
\label{fig:r006-continuum-template}
\end{figure}

\section{Motivation: Why a Shared Systems Language Is Needed}
\label{sec:r006-motivation}
...
the opening pages do not explain the problem, the payoff, and the boundary of
...
\label{sec:r006-conceptual-background}
