...
integrates it with the declared hierarchy K0--K12, the birth
operators \(\Psi_{x\to x+1}\) (the operator semantics chapter) and the
collapse–rebirth structure (the lifecycle and rebirth chapter).
...
\(\Psi_{x\to x+1}\) (the operator semantics chapter).
...
\begin{enumerate}
  ...
  \item \textbf{Birth compatibility.}\quad
        ...
        conditions of the operator semantics chapter;
  \item \textbf{Axis monotonicity.}\quad
        ...
        \]
        and each new axis is not representable in the structural span of
        previous axes;
...
Let \(\mathcal{K}\) be the class of declared continuum classes admissible under the OC
...
\begin{enumerate}
  \item \textbf{Distinct new axes.}\quad
        ...
        not mutually representable in the structural span of one another, i.e.
        \[
...
\]
Core~1.3.3 treats \(\Theta_{\mathrm{branch}}\) as a derived quantity rather
...
(the operator semantics chapter).
...
\begin{itemize}
  ...
  \item \textbf{Fusion.}\quad
        Through strong coupling and shared boundary structures, two branches
        can generate a new continuum that lies on a common extension branch.
        Formally this corresponds to a composite birth operator fed by
        both \(B^{(a)}\) and \(B^{(b)}\).
\end{itemize}
...
individual continua (the lifecycle and rebirth chapter) to whole development
paths.
...
\begin{enumerate}
  ...
  \item there exists no continuum \(K_{x_{n+1}}\) with \(x_{n+1}>x_n\)
        ...
        defined on any residue of \(K_{x_n}\) (the lifecycle and rebirth chapter);
...
The branch structure refines the declared hierarchy K0--K12 by
...
(the operator semantics chapter), the branch structure enforces vertical
coherence:
\begin{itemize}
...
Taken together, the declared hierarchy K0--K12, the threshold
landscape, and the branch topology form the global structural picture of OC.
The operative machinery is carried by \(F\), \(G\), \(H\), \(Q\), \(R\),
\(S\), \(U\), \(\Psi\), and \(E_{\mathrm{int}}\):
\begin{itemize}
...
Core~1.3.3 fixes this global picture at the structural level. Subsequent
extensions and domain-specific studies may quantify branch
