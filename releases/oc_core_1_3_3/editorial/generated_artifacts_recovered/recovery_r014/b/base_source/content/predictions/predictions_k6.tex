...
\subsubsection{Bounded Prediction Review for \texorpdfstring{$K_6$}{K_6}}
\label{sec:predictions-k6-r014}

At \(K_6\), prediction language is treated as a source-grounded review
target for cognitive and representational systems, not as an unrestricted theorem about every system in that
...
\paragraph{Claim.}
A \(K_6\) prediction may be considered only under declared assumptions
...
A safe \(K_6\) row has the form
\[
  \mathcal{P}_{6}:
  (K_6, A_x, \Theta_x, P_x, J_x, C_x, k_x, M_x)
  \longrightarrow
...
For \(K_6\), the current release records prediction rows as bounded
