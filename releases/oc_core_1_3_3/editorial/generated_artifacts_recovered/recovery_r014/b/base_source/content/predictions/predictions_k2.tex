...
\subsubsection{Bounded Prediction Review for \texorpdfstring{$K_2$}{K_2}}
\label{sec:predictions-k2-r014}

At \(K_2\), prediction language is treated as a source-grounded review
target for physical field and spacetime models, not as an unrestricted theorem about every system in that
...
\paragraph{Claim.}
A \(K_2\) prediction may be considered only under declared assumptions
...
A safe \(K_2\) row has the form
\[
  \mathcal{P}_{2}:
  (K_2, A_x, \Theta_x, P_x, J_x, C_x, k_x, M_x)
  \longrightarrow
...
For \(K_2\), the current release records prediction rows as bounded
