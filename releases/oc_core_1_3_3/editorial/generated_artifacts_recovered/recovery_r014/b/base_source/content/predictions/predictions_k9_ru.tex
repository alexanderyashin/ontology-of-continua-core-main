...
\subsubsection{Bounded Prediction Review for \texorpdfstring{$K_9$}{K_9}}
\label{sec:predictions-k9-r014}

At \(K_9\), prediction language is treated as a source-grounded review
target for theory and knowledge systems, not as an unrestricted theorem about every system in that
...
\paragraph{Claim.}
A \(K_9\) prediction may be considered only under declared assumptions
...
A safe \(K_9\) row has the form
\[
  \mathcal{P}_{9}:
  (K_9, A_x, \Theta_x, P_x, J_x, C_x, k_x, M_x)
  \longrightarrow
...
For \(K_9\), the current release records prediction rows as bounded
