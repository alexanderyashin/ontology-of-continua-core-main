...
\subsubsection{Bounded Prediction Review for \texorpdfstring{$K_3$}{K_3}}
\label{sec:predictions-k3-r014}

At \(K_3\), prediction language is treated as a source-grounded review
target for chemical reaction and molecular systems, not as an unrestricted theorem about every system in that
...
\paragraph{Claim.}
A \(K_3\) prediction may be considered only under declared assumptions
...
A safe \(K_3\) row has the form
\[
  \mathcal{P}_{3}:
  (K_3, A_x, \Theta_x, P_x, J_x, C_x, k_x, M_x)
  \longrightarrow
...
For \(K_3\), the current release records prediction rows as bounded
