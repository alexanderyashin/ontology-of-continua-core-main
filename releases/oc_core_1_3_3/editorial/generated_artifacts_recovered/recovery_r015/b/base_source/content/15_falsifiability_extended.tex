...
\(K_2\)–\(K_{12}\), using only the axioms and definitions of Core~1.3.3.
...
\begin{itemize}
  ...
  \item \emph{Structural falsifiability} concerns violations of the scoped
...
\begin{enumerate}
  ...
  \item \textbf{Inconsistent tuples} are required:
        successful models require mutually incompatible versions of the
        canonical OC state tuple for one and the same system.
\end{enumerate}
...
Because OC claims bounded cross-domain applicability where assumptions and evidence hold, a structural falsification in one domain
(propely established) is routed across declared domain packets that rely on the same
...
\begin{itemize}
  ...
  \item \textbf{Experimental programme.}
        ...
        \(\partial\Omega_2\) behaves as the OC model would predict under declared assumptions (critical slowing down,
...
Core 1.3.3 records the current route for algorithmic tests for:
\begin{itemize}
...
Core 1.3.3 states the current structural basis for an explicit validation pipeline:
\begin{enumerate}
