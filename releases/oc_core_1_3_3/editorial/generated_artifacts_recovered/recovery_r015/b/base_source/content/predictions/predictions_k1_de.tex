...
\subsubsection{Bounded Prediction Review for \texorpdfstring{$K_1$}{K_1}}
\label{sec:predictions-k1-r014}

At \(K_1\), prediction language is treated as a source-grounded review
target for primitive ordering and one-axis continuity, not as an unrestricted theorem about every system in that
...
\paragraph{Claim.}
A \(K_1\) prediction may be considered only under declared assumptions
...
A safe \(K_1\) row has the form
\[
  \mathcal{P}_{1}:
  (K_1, A_x, \Theta_x, P_x, J_x, C_x, k_x, M_x)
  \longrightarrow
...
For \(K_1\), the current release records prediction rows as bounded
