...
\subsubsection{Bounded Prediction Review for \texorpdfstring{$K_5$}{K_5}}
\label{sec:predictions-k5-r014}

At \(K_5\), prediction language is treated as a source-grounded review
target for excitable bioelectrical systems, not as an unrestricted theorem about every system in that
...
\paragraph{Claim.}
A \(K_5\) prediction may be considered only under declared assumptions
...
A safe \(K_5\) row has the form
\[
  \mathcal{P}_{5}:
  (K_5, A_x, \Theta_x, P_x, J_x, C_x, k_x, M_x)
  \longrightarrow
...
For \(K_5\), the current release records prediction rows as bounded
