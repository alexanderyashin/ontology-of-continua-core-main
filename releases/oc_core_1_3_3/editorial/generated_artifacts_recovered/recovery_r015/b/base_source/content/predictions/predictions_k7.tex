...
\subsubsection{Bounded Prediction Review for \texorpdfstring{$K_7$}{K_7}}
\label{sec:predictions-k7-r014}

At \(K_7\), prediction language is treated as a source-grounded review
target for social and institutional systems, not as an unrestricted theorem about every system in that
...
\paragraph{Claim.}
A \(K_7\) prediction may be considered only under declared assumptions
...
A safe \(K_7\) row has the form
\[
  \mathcal{P}_{7}:
  (K_7, A_x, \Theta_x, P_x, J_x, C_x, k_x, M_x)
  \longrightarrow
...
For \(K_7\), the current release records prediction rows as bounded
