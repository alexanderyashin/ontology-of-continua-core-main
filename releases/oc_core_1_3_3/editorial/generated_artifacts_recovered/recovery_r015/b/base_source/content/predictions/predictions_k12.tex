...
\subsubsection{Bounded Prediction Review for \texorpdfstring{$K_12$}{K_12}}
\label{sec:predictions-k12-r014}

At \(K_12\), prediction language is treated as a source-grounded review
target for upper coherence and compatibility systems, not as an unrestricted theorem about every system in that
...
\paragraph{Claim.}
A \(K_12\) prediction may be considered only under declared assumptions
...
A safe \(K_12\) row has the form
\[
  \mathcal{P}_{12}:
  (K_12, A_x, \Theta_x, P_x, J_x, C_x, k_x, M_x)
  \longrightarrow
...
For \(K_12\), the current release records prediction rows as bounded
