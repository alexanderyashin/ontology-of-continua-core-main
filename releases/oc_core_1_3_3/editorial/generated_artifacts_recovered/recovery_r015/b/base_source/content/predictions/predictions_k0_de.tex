...
\subsubsection{Bounded Prediction Review for \texorpdfstring{$K_0$}{K_0}}
\label{sec:predictions-k0-r014}

At \(K_0\), prediction language is treated as a source-grounded review
target for resolution-relative distinguishability, not as an unrestricted theorem about every system in that
...
\paragraph{Claim.}
A \(K_0\) prediction may be considered only under declared assumptions
...
A safe \(K_0\) row has the form
\[
  \mathcal{P}_{0}:
  (K_0, A_x, \Theta_x, P_x, J_x, C_x, k_x, M_x)
  \longrightarrow
...
For \(K_0\), the current release records prediction rows as bounded
