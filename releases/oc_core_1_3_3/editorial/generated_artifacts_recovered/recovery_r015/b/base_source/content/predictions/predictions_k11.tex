...
\subsubsection{Bounded Prediction Review for \texorpdfstring{$K_11$}{K_11}}
\label{sec:predictions-k11-r014}

At \(K_11\), prediction language is treated as a source-grounded review
target for recursive model-space systems, not as an unrestricted theorem about every system in that
...
\paragraph{Claim.}
A \(K_11\) prediction may be considered only under declared assumptions
...
A safe \(K_11\) row has the form
\[
  \mathcal{P}_{11}:
  (K_11, A_x, \Theta_x, P_x, J_x, C_x, k_x, M_x)
  \longrightarrow
...
For \(K_11\), the current release records prediction rows as bounded
