...
\begin{enumerate}
    \item a new class of differences arises that cannot be expressed within
...
These flows feed into the general evolution operators \(F, G, H, Q, R, S, U\), which specify how axes, potentials, thresholds, flows, cycles, and boundaries co-evolve.

\subsection{Continuumness}

The measure \(k(K,t)\) determines whether a system functions as a \emph{live continuum}.
Core~1.3.3 adopts the unified definition encoded by the operator \(U\):
...
Core~1.3.3 reserves \(\chi_{\Omega}(s;K,t)\) for pointwise membership of a
state \(s\) in the admissible region. Nonemptiness of the whole admissible
region is therefore always written with
\(\mathbf{1}_{\{\Omega(K,t)\neq\emptyset\}}\), not with \(\chi_{\Omega}\).
...
\subsection{Motivation for Core~1.3.3}

Core~1.3.3 consolidates all structural components of OC into a vertically
...
\begin{itemize}
    ...
    \item integrate the full hierarchy \(K_0\)–\(K_{12}\) together with the embedding meta-spaces \(M_x\).
\end{itemize}
...
\begin{itemize}
    ...
    \item derivations of the general operators \(F,G,H,Q,R,S,U\) from more primitive assumptions.
\end{itemize}

Core~1.3.3 keeps this chapter deliberately compact: it supplies the background
needed for the formal model chapter without duplicating the later proof,
operational, and unified-synthesis chapters.
