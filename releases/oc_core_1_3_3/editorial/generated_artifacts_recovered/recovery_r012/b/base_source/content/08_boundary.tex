...
% Extended Boundary and Patch Geometry — Core 1.3.3
% ====================================================================
...
Boundaries constitute one of the most declared model structures in the OC framework:
...
\(K_6\)--\(K_8\).
The present version consolidates the working structure needed for Core~1.3.3:
...
\]
Equivalently, if \(f_k(s)=g_k(s)-\Theta_k\) denotes the shifted residual form
used in the notation appendix and the model chapter, then interior states
satisfy \(f_k(s)<0\), admissible-side states satisfy \(f_k(s)\le0\), and the
same boundary is the zero set \(f_k(s)=0\) for at least one active threshold.
...
and flows.
If the active threshold at a point \(s \in \partial\Omega(K)\) is \(g_k\), the
...
Across \(K_2\)--\(K_5\) three threshold classes recur across declared cases.
