...
% ---------------------------------------------------------------
\subsection{Role of Predictions Within Core 1.3}
...
\[
P_i \text{ false } \Rightarrow \text{the relevant bounded OC claim must be revised}.
\]
...
\subsection{Claim Ceiling for Prediction Language}
\label{sec:predictions-claim-ceiling-v3}

In this release, OC prediction language has a claim ceiling. Structural
signatures and diagnostic classifications are allowed when the relevant
operators, thresholds, and observables are named. Quantitative prediction is
allowed only when the release supplies the target variable, data route, split
policy, formula or executable procedure, output value or interval, comparator,
and falsifier. Without those artifacts, a row is a structural or diagnostic
claim, not a numerical prediction.
