...
Each domain-specific mapping is treated here as a bounded embedding criterion
rather than as an unrestricted identification theorem. The aim is to show how
well-studied classes of systems can be represented as continua under OC's
structural axioms, not to claim that every object in a discipline is exhausted
by a single OC encoding \parencite{anderson_more_is_different}. Operational
advantage claims, benchmark superiority claims, and practical deployment
verdicts are intentionally deferred to Chapter~26, whose role is to compare
...
\paragraph{Embedding criterion (Physics → \texorpdfstring{\(K_2\)}{K2}).}
...
\(K_4\).
The mapping is bounded to reaction-network closure, catalytic organisation and
...
\paragraph{Embedding criterion (Chemistry/Protocells → \texorpdfstring{\(K_3\)}{K3}/\texorpdfstring{\(K_4\)}{K4}).}
...
% ====================================================================
\subsection{Biology (Levels \texorpdfstring{$K_4$}{K4}--\texorpdfstring{$K_5$}{K5})}
% ====================================================================

Biological cells emerge when protocells acquire excitability and structured
regulation.
Here the target is bounded excitable and morphogenetic organisation rather
...
\paragraph{Embedding criterion (Biology → \texorpdfstring{\(K_5\)}{K5}).}
...
\paragraph{Embedding criterion (Cognition → \texorpdfstring{\(K_6\)}{K6}).}
...
\paragraph{Embedding criterion (Society → \texorpdfstring{\(K_7\)}{K7}).}
...
% ====================================================================
\subsection{Civilisation (Level \texorpdfstring{$K_8$}{K8})}
% ====================================================================

Civilizational systems extend social continua to encompass infrastructures,
energy flows and large-scale coordination.
The scope is bounded to large-scale organised systems with networked
...
\paragraph{Embedding criterion (Civilisation → \texorpdfstring{\(K_8\)}{K8}).}
...
\paragraph{Embedding criterion (Theory → \texorpdfstring{\(K_9\)}{K9}, Meta-Theory → \texorpdfstring{\(K_{10}\)}{K10}).}
...
This chapter establishes bounded cross-disciplinary embedding criteria across
physics, chemistry, biology, cognition, society, civilisation, and formal
theory. Each lane is mapped to the same structural tuple, but each mapping is
...
harder support rules of Chapter~26.
