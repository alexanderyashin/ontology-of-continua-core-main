...
Abschnitt~\ref{sec:model} eingeführten abstrakten Evolutionsoperator
zu der vollständigen Operatorfamilie, die in der Ontology of
Continua (OC) verwendet wird. Es fasst die Definitionen aus Core~2.x
zusammen und stellt sie in einer kompakten, vertikal konsistenten Form dar,
die für Core~1.3 geeignet ist.
...
\[
  ...
  \big),
\]
mit den in Abschnitt~\ref{sec:model} definierten Komponenten geschrieben.
...
Abschnitt~\ref{sec:results}, dass der Tod mit \(C_{\max}(t) = \emptyset\)
...
Abschnitt~\ref{sec:boundary-extended} enthält eine erweiterte Darstellung der
...
Abschnitt~\ref{sec:birth-operators} formalisiert.
...
Mit der vereinheitlichten Definition von \(k(t)\) aus Abschnitt~\ref{sec:model}
...
Dieses Kapitel vervollständigt den formalen Kern von Core~1.3, indem es die
