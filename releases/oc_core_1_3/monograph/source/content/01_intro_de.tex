...
% THEOREM_REVIEW_DOSSIER_REF: logion/k0/governance/status/external_llm_review_dossiers/OC_CORE_1_3_MONOGRAPH_THEOREM_REVIEW__DE__content_01_intro_de_tex.json
% SOURCE_LANGUAGE: EN
% TARGET_LANGUAGE: DE
...
\emph{Ontology of Continua v3.0 — Core~1.3} bietet die erste vollständig konsolidierte und vertikal kohärente Referenz für die Ontology of Continua (OC). Core~1.3 führt keine neuen Axiome ein; stattdessen ordnet und harmonisiert es alle zuvor genehmigten Komponenten zu einer einzigen stabilen Veröffentlichung neu. Dazu gehören die Axiomatik von \(K_0\), die explizite Konstruktion von \(K_1\), die allgemeine Definition eines Kontinuums, die Taxonomie der Schwellen, die formale Behandlung von Potentialen und Flüssen, die strukturellen Bedingungen für Geburt, Leben und Tod, die vollständige vertikale Hierarchie \(K_0\)–\(K_{10}\) sowie die Einbettungsrolle der Metaräume \(M_x\) mit ihrem Erweiterungsoperator \(\Psi\).
...
\subsection{Umfang des öffentlichen Core}

Core~1.3 verfolgt einen klar umrissenen Zweck: den grundlegenden Formalismus zu stabilisieren. Seine Aufgaben sind:
...
Core~1.3 erweitert die Hierarchie nicht über \(K_{10}\) hinaus; seine Rolle besteht darin, die Definitionen, Grenzen, Schwellen und Übergänge für die vorhandenen Ebenen zu klären und zu stabilisieren.

\subsection{Bezug zu früheren Versionen}

Frühere Versionen des OC-Kerns existierten in fragmentierten internen Dokumenten. Core~1.3 konsolidiert sie zu einer einzigen kohärenten Referenz. Die Ziele sind:
...
Die Theorie selbst bleibt unverändert; Core~1.3 ist eine Fassung zur Klärung und Stabilisierung.

\subsection{Aufbau der Monographie}

Diese Monographie ist wie folgt aufgebaut.  
Abschnitt~\ref{sec:background} führt in den minimalen strukturellen und mathematischen Hintergrund ein.  
Abschnitt~\ref{sec:model} präsentiert den formalen Kern: die Axiome von \(K_0\), die Konstruktion von \(K_1\), die allgemeine Definition des Kontinuums, die Schwellentaxonomie, Potentiale, Flüsse sowie die Evolutions- und Interaktionsoperatoren.  
Abschnitt~\ref{sec:results} leitet die wichtigsten strukturellen Konsequenzen her: Monotonie der Dimension, Unmöglichkeit spontaner Dimensionsentstehung, schwelleninduzierte Emergenz und Irreversibilität des Kollapses.  
Abschnitt~\ref{sec:discussion} diskutiert Interpretation, Umfang und Grenzen.  
Abschnitt~\ref{sec:conclusion} schließt ab und skizziert künftige Erweiterungen.

Core~1.3 ist als eigenständige Referenz gedacht. Für Anwendungen interessierte Leserinnen und Leser können technische Details überspringen; wer sich auf die formale Struktur konzentriert, kann den Definitionen und Konsequenzen unmittelbar folgen.
