\section{Foundational Consistency Dossier}
...
\(K_{11}\) is the level at which recursive meta-theoretical continua govern
operator families, meta-logical update, and model-space recursion. Theoretical
continua comprising theories, paradigms, ontologies, and model families remain
the \(K_9\) layer; \(K_{11}\) is needed only when the theory studies the
evolution of the spaces in which theories and theory-building operators live.
...
claim-scope taxonomy; Section~\ref{sec:oc13-theorem-roadmap} adds the
...
\item \textbf{Empirical statements} require a standalone source-owned domain
      packet, canonical theorem-native trace, pinned data route, numerical
...
\item an explicit contradiction of a promoted claim first flags a
      refutation-candidate or surface-contradiction case for SPOT audit;
...
the canonical SPOT scientific-class and closure-verdict vocabulary. Release
verdicts such as \texttt{PASS} and \texttt{FAIL\_CLOSED} remain a separate
verdict layer. The mapping is:
\begin{itemize}
\item Axiomatic statements are admitted premises or identity rules. They may
      sit inside a \occode{THEOREM_NATIVE} route as named assumptions, but
      the axiom itself is not thereby converted into a proved theorem.
\item Derived statements map to \occode{THEOREM_NATIVE} when the support
      route is closed.
\item Empirical statements map to \occode{THEOREM_NATIVE} only after the
      source-owned packet, theorem-native trace, pinned data route, numerical
      parameter law, observable binding, locked held-out replay procedure,
...
empirical promotions. \texttt{BRIDGE\_ONLY} rows are scope-bounded evidence
support rather than public promoted closure; they are not re-labelled as
theorem-native \texttt{PASS} rows unless a separate source-owned
theorem-native packet is created and passes the canonical gates. \texttt{EXTENSION} and
\texttt{REFUTED} lanes remain outside public \texttt{PASS} authority.
...
levels. The short falsifier used in this section is shorthand for the fuller
preservation burden used in Section~\ref{sec:oc13-worked-examples}: a proposed
level fails only if its objects, axes, thresholds, potentials or flows, cycles,
embedding-space distinctions, admissibility and collapse conditions, and
level-specific proof obligations can be translated into an adjacent level
without loss of explanatory or operational burden.  For the upper levels this
includes the K10-only preservation test, the K11 object-evolution and
meta-theoretical split, and the K12 global compatibility and integrability
conditions.
...
families of such \(K_{11}\)-objects. The promoted theorem boundary is therefore
specific: the K11/K12 fork is necessary beyond a K10-only hierarchy for the
current source corpus. The stronger exact-cardinality claim that exactly
thirteen levels, and no other lawful refinement, are necessary is not promoted.
Such a claim would require a separate component-by-component reduction program
for every level boundary; until that program is complete, the current public
release presents the K0--K12 hierarchy as the audited hierarchy of the Core~1.3
source series, not as a closed minimality theorem over all possible continuum
hierarchies.
