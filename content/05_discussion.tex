...
\[
  K = \big(\Omega(K), A(K), P(t), J(t), \Theta(K), \partial\Omega(K), C(K), k(t)\big),
\]
where:
\begin{itemize}
    ...
    \item \(k(t)\) is the measure of continuumness.
\end{itemize}
...
The measure \(k(t)\) integrates information about the richness of \(\Omega(K)\), the presence and stability of cycles, expressive adequacy of axes and compatibility with thresholds.
...
\begin{itemize}
    ...
    \item \textbf{Threshold–based emergence.}
          Theorems~2, 6 and 11 encode the idea that emergence is discrete and threshold–driven:
          new structure appears only when structural tension exceeds dimensional thresholds and the embedding space has a suitable axis available.
...
cycles and the measure \(k(t)\) capture the conditions for structural persistence; 
