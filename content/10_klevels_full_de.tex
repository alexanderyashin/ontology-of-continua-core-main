...
\section{Full Hierarchy of Continua \texorpdfstring{\texorpdfstring{$K_0$}{K\_0}–$K_{12}$}{K0–K12}}
\label{sec:klevels-full}

This chapter provides the complete reconstructed description of all
continuum levels \(K_0\)–\(K_{12}\), integrating the material originally
distributed across Core~2.x and the Physics, Chemistry, Biology, Cognition
and Social runs.
...
Core~1.3 fixes the hierarchy as \(K_0\)–\(K_{12}\) because the öffentlich source
...
A global Zusammenfassung is shown in Table~\ref{tab:klevels-Zusammenfassung}.
...
\hline
\end{tabular}
\caption{Global overview of continua \texorpdfstring{\(K_0\)}{K0}–\(K_{12}\).}
\label{tab:klevels-Zusammenfassung}
\end{table}
...
This completes the reconstructed full hierarchy of continua from
\(K_0\) through \(K_{12}\).
