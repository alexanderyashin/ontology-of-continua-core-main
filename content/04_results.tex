...
They follow solely from the structural definitions of continua, axes, thresholds, potentials, flows, boundaries, cycles and the measure of continuumness \(k(t)\).
...
The dimensional threshold \(\Theta_{\mathrm{dim}}\) enforces that \(A_{\mathrm{new}}\) is nondegenerate and structurally necessary.
...
if \(K(t)\) is live (\(k(t) > 0\)), then
\[
    \dim(K(t+dt)) \ge \dim(K(t)).
\]
...
\[
    T(K) > \Theta_{\mathrm{dim}}(K)
    \quad\Rightarrow\quad
    \text{only then can a new axis emerge.}
\]

\textbf{Justification (sketch).}
New axes represent classes of differences that are incompatible with existing axes.
Such incompatibility arises only when structural tension \(T\) with respect to the current threshold landscape cannot be resolved within the existing axes.
By definition of \(\Theta_{\mathrm{dim}}\), below this threshold all differences can be projected onto the span of \(A(K)\); above it, projection fails and a new axis is required.

\textbf{Consequence 2.1.}
Noise, local fluctuations or internal flows that do not raise structural tension above \(\Theta_{\mathrm{dim}}\) cannot generate new dimensions.
...
\[
    A(K_x(t)) \subseteq A(M_x),
    \qquad
    \mathrm{span}\,A(K_x(t+dt)) \subseteq \mathrm{span}\,A(M_x).
\]
...
\subsection{Theorem 5: Necessity and sufficiency of compatibility with \texorpdfstring{$M$}{M}}

\textbf{Statement.}
A continuum \(K\) exists (i.e. \(\Omega(K)\neq\emptyset\) and \(k>0\)) if and only if it is compatible with its embedding space \(M\):
\[
    K \text{ exists }
    \Longleftrightarrow
    \Omega(K) \neq \emptyset
...
Incompatible configurations are excluded by setting \(\Omega(K) = \emptyset\).

\subsection{Theorem 6: Structural tension and phase transitions}

\textbf{Statement.}
Let \(T(K)\) denote structural tension, understood as a functional of potentials, axes and their gradients:
\[
    T = T(P,A,\nabla P).
\]
Then:
...
\[
    S(K(t+dt)) \ge S(K(t)),
\]
where \(S\) is a structural measure of complexity defined on the state space and its organisation (e.g. combining contributions from number of axes, diversity of states, richness of cycles and structure of embedding).
...
\end{itemize}
Stagnant or constant complexity corresponds to finely balanced dynamics below critical thresholds; however, this regime is generically unstable (see Theorem~8).
...
\begin{itemize}
    ...
    \item continuumness \(k(t)\) decays to zero.
\end{itemize}
...
\[
    \dim(A(K)) < \dim\big(\mathrm{Differences}(K)\big),
\]
