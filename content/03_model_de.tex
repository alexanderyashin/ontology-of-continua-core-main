...
% SOURCE_EN_REF: content/03_model.tex

\subsection{Primitive versus derived tuple roles}
\label{sec:oc13-tuple-primitive-derived-v3}

The tuple \(K=(\Omega,A,P,J,\Theta,\partial\Omega,C,k)\) is the public interface
used by this release. It is not claimed here to be a proved minimal or complete
canonical form. In particular, \(\partial\Omega\) is derived when boundary
functions for \(\Omega\) and \(\Theta\) are supplied; \(J\) may be derived from
or constrained by \(P\) in local models; and \(k\) is a diagnostic functional
whose formula and calibration must be declared before it carries quantitative
force. The primitive/derived split is therefore part of the proof obligation,
not an afterthought.
